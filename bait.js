...
function bait__gen__c__Gen_struct_init(g, node) {
	...
	for (let _t787 = 0; _t787 < node.fields.length; _t787++) {
		const field = Array_get(node.fields, _t787)
...
function bait__gen__c__Gen_typ(g, typ) {
	...
	let _t795 = undefined
	if (eq(sym.kind, bait__ast__TypeKind.enum_)) {
		_t795 = from_js_string("enum ")
	} else {
		_t795 = from_js_string("")
	}
	const prefix = _t795
...
function bait__gen__c__Gen_write_types(g) {
	for (let _t795 = 0; _t795 < g.table.type_symbols.length; _t795++) {
		const sym = Array_get(g.table.type_symbols, _t795)
		...
		if (sym.info instanceof bait__ast__StructInfo) {
			...
			for (let _t797 = 0; _t797 < info.fields.length; _t797++) {
				const field = Array_get(info.fields, _t797)
			...
			for (let _t799 = 0; _t799 < info.vals.length; _t799++) {
				const val = Array_get(info.vals, _t799)
...
function bait__gen__c__Gen_stmts(g, stmts) {
	bait__gen__c__Gen_save_stmt_offset(g)
	g.indent += 1
	for (let _t803 = 0; _t803 < stmts.length; _t803++) {
		const stmt = Array_get(stmts, _t803)
...
function bait__gen__c__gen(files, table, pref) {
	...
	for (let _t813 = 0; _t813 < files.length; _t813++) {
		const file = Array_get(files, _t813)
		g.path = file.path
		g.pkg = file.pkg_name
	...
	for (let _t814 = 0; _t814 < g.table.needed_str_funs.length; _t814++) {
		const typ = Array_get(g.table.needed_str_funs, _t814)
...
function bait__gen__c__Gen_process_imports(g, imports) {
	for (let _t814 = 0; _t814 < imports.length; _t814++) {
		const imp = Array_get(imports, _t814)
...
function bait__gen__c__Gen_headers(g) {
	...
	for (let _t815 = 0; _t815 < g.foreign_imports.length; _t815++) {
		const name = Array_get(g.foreign_imports, _t815)
...
function bait__gen__c__Gen_get_concrete_name(g, name, concrete_types) {
	let full_name = name
	for (let _t816 = 0; _t816 < concrete_types.length; _t816++) {
		const t = Array_get(concrete_types, _t816)
		full_name = string_add(full_name, string_add(from_js_string("_"), bait__ast__Table_get_sym(g.table, t).mix_name))
	}
...
function bait__gen__c__Gen_cut_before_any(g, chars) {
	for (let i = i32(g.out.length - 1); i32(i >= 0); i -= 1) {
		for (let _t818 = 0; _t818 < chars.length; _t818++) {
			const c = string_get(chars, _t818)
			if (eq(string_get(g.out, i), c)) {
				let cut = string_substr(g.out, i32(i + 1), g.out.length)
				g.out = string_substr(g.out, 0, i)
				return cut
...
function bait__gen__c__Gen_gen_equality_funs(g) {
	for (let _t820 = 0; _t820 < g.table.needed_equality_funs.length; _t820++) {
		const typ = Array_get(g.table.needed_equality_funs, _t820)
		_t821 = bait__gen__c__Gen_equality_fun(g, typ)
	}
}
...
function bait__transformer__Transformer_transform_files(t, files) {
	for (let _t828 = 0; _t828 < files.length; _t828++) {
		let file = Array_get(files, _t828)
		t.path = file.path
...
function bait__transformer__Transformer_for_classic_loop(t, node) {
	_t836 = bait__transformer__Transformer_stmt(t, node.init)
	_t837 = bait__transformer__Transformer_stmt(t, node.inc)
	bait__transformer__Transformer_stmts(t, node.stmts)
}
...
function bait__transformer__Transformer_if_match(t, node) {
	for (let _t838 = 0; _t838 < node.branches.length; _t838++) {
		let branch = Array_get(node.branches, _t838)
		_t839 = bait__transformer__Transformer_expr(t, branch.cond)
		bait__transformer__Transformer_stmts(t, branch.stmts)
	}
}

function bait__transformer__Transformer_return_stmt(t, node) {
	_t840 = bait__transformer__Transformer_expr(t, node.expr)
}

function bait__transformer__Transformer_call_expr(t, node) {
	_t841 = bait__transformer__Transformer_expr(t, node.left)
	for (let _t841 = 0; _t841 < node.args.length; _t841++) {
		let arg = Array_get(node.args, _t841)
...
function bait__transformer__Transformer_gen_test_main(t) {
	...
	for (let _t847 = 0; _t847 < t.gen_ctx.test_fun_names.length; _t847++) {
		const name = Array_get(t.gen_ctx.test_fun_names, _t847)
...
function bait__builder__run_tests(prefs) {
	...
	for (let _t850 = 0; _t850 < prefs.args.length; _t850++) {
		const a = Array_get(prefs.args, _t850)
...
function bait__builder__Builder_bait_files_in_dir(b, dir) {
	...
	for (let _t854 = 0; _t854 < all_files.length; _t854++) {
		const f = Array_get(all_files, _t854)
...
function bait__builder__compile(prefs) {
	...
	for (let _t857 = 0; _t857 < paths.length; _t857++) {
		const p = Array_get(paths, _t857)
	...
	for (let i = 0; i32(i < ast_files.length); i += 1) {
		...
		for (let _t857 = 0; _t857 < file.imports.length; _t857++) {
			const imp = Array_get(file.imports, _t857)
			...
			for (let _t860 = 0; _t860 < imp_paths.length; _t860++) {
				const p = Array_get(imp_paths, _t860)
	...
	if (i32(b.checker.errors.length > 0)) {
		for (let _t864 = 0; _t864 < b.checker.errors.length; _t864++) {
			const err = Array_get(b.checker.errors, _t864)
	...
	for (let _t864 = 0; _t864 < ast_files.length; _t864++) {
		const f = Array_get(ast_files, _t864)
		for (let _t864 = 0; _t864 < f.imports.length; _t864++) {
			const imp = Array_get(f.imports, _t864)
			if (!eq(imp.lang, bait__ast__Language.bait)) {
				continue
			}
	...
	for (let _t865 = 0; _t865 < pkg_order.length; _t865++) {
		const pkg = Array_get(pkg_order, _t865)
		for (let _t865 = 0; _t865 < ast_files.length; _t865++) {
			const f = Array_get(ast_files, _t865)
...
function bait__builder__Builder_print_infos(b, infos) {
	...
	for (let _t878 = 0; _t878 < infos.length; _t878++) {
		const info = Array_get(infos, _t878)
...
function bait__builder__Builder_print_errors_and_warnings(b, parser_errs) {
	let nr_warns = 0
	let nr_errors = 0
	for (let _t878 = 0; _t878 < b.parsed_files.length; _t878++) {
		const f = Array_get(b.parsed_files, _t878)
		nr_warns += f.warnings.length
		nr_errors += f.errors.length
		bait__builder__Builder_print_infos(b, f.infos)
		if (b.prefs.warn_is_error) {
			for (let _t879 = 0; _t879 < f.warnings.length; _t879++) {
				const warn = Array_get(f.warnings, _t879)
		...
		for (let _t879 = 0; _t879 < f.errors.length; _t879++) {
			const err = Array_get(f.errors, _t879)
			bait__errors__Message_print(err)
			if (parser_errs) {
				return true
			}
	...
	for (let _t880 = 0; _t880 < b.checker.errors.length; _t880++) {
		const err = Array_get(b.checker.errors, _t880)
		bait__errors__Message_print(err)
	}
...
function bait__builder__order_pkgs(ordered, pkg, deps, looked) {
	...
	for (let _t885 = 0; _t885 < Map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })).length; _t885++) {
		const d = Array_get(Map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })), _t885)
...
const bait__util__FULL_VERSION = from_js_string(`${bait__util__VERSION.str} ${from_js_string("4e9fa3e").str}`)

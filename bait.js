...
function TestRunner_assert_fail(tr) {
	tr.fails += 1
	eprintln(from_js_string(`${tr.file.str}:${i32_str(tr.line)} ${tr.fun_name.str}`).str)
...
function bait__ast__Import({ name = from_js_string(""), alias = from_js_string(""), pos = new bait__token__Pos({}) }) {
	this.name = name
	this.alias = alias
	this.pos = pos
}
...
function array_bait__ast__Attribute_find_attr(attrs, name) {
	for (let _t2 = 0; _t2 < attrs.length; _t2++) {
		const attr = array_get(attrs, _t2)
...
function bait__ast__Table_fun_type_signature(t, param_types, return_type) {
	...
	for (let _t3 = 0; _t3 < param_types.length; _t3++) {
		const param = array_get(param_types, _t3)
...
function bait__ast__Table_get_method(t, sym, name) {
	let s = sym
	while (true) {
		for (let _t4 = 0; _t4 < s.methods.length; _t4++) {
			const m = array_get(s.methods, _t4)
...
function bait__ast__TypeSymbol({ name = from_js_string(""), kind = 0, info = undefined, methods = new array({ data: [], length: 0 }), parent = 0, overloads = new map({ data: new Map([]), length: 0 }), is_pub = false, pkg = from_js_string("") }) {
	this.name = name
	this.kind = kind
	this.info = info
	this.methods = methods
...
function bait__ast__TypeSymbol_find_field(sym, name, t) {
	if (sym.info instanceof bait__ast__StructInfo) {
		const info = sym.info
		for (let _t5 = 0; _t5 < info.fields.length; _t5++) {
			const f = array_get(info.fields, _t5)
...
function bait__ast__TypeSymbol_has_method(sym, name) {
	for (let _t6 = 0; _t6 < sym.methods.length; _t6++) {
		const m = array_get(sym.methods, _t6)
		if (string_eq(m.name, name)) {
			return true
		}
	}
...
function bait__parser__Parser_anon_fun(p) {
	...
	for (let _t7 = 0; _t7 < params.length; _t7++) {
		const param = array_get(params, _t7)
...
function bait__parser__Parser_fun_call(p) {
	const pos = p.tok.pos
	...
	bait__parser__Parser_check(p, bait__token__TokenKind.rpar)
	return new bait__ast__CallExpr({ name: name, args: args, pkg: p.pkg_name, pos: pos })
}
...
function bait__parser__Parser_name_expr(p) {
	...
	if (p.next_tok.kind == bait__token__TokenKind.lpar) {
		return bait__parser__Parser_fun_call(p)
	}
...
function bait__parser__Parser_import_stmts(p) {
	...
	while (p.tok.kind == bait__token__TokenKind.key_import) {
		...
		array_push(imports, new bait__ast__Import({ name: name, alias: alias, pos: pos }))
	}
	return imports
}
...
function bait__parser__parse(tokens, path, table, pref) {
	...
	while (p.tok.kind != bait__token__TokenKind.eof) {
		if (p.tok.kind == bait__token__TokenKind.attr) {
...
function bait__parser__Parser_parse_attributes(p) {
	while (p.tok.kind == bait__token__TokenKind.attr) {
		const pos = p.tok.pos
		const name = p.tok.val
		bait__parser__Parser_next(p)
		let value = from_js_string("")
		if (p.tok.kind == bait__token__TokenKind.colon) {
			bait__parser__Parser_next(p)
...
function bait__parser__Parser_prepend_lang(p, lang, val) {
	if (lang == bait__ast__Language.bait) {
		return val
	}
...
function bait__parser__Parser_fun_decl(p) {
	...
	let node = new bait__ast__FunDecl({ is_test: p.is_test_file && string_starts_with(name, from_js_string("test_")), is_pub: is_pub, name: name, params: params, return_type: return_type, attrs: p.attributes, pos: pos })
...
function bait__parser__Parser_interface_decl(p) {
	...
	while (p.tok.kind != bait__token__TokenKind.rcur) {
		...
		if (p.tok.kind == bait__token__TokenKind.name) {
...
function bait__parser__Parser_parse_map_type(p) {
	...
	if (key_type != bait__ast__STRING_TYPE) {
...
const bait__checker__KNOWN_ATTRS = bait__checker__build_known_attrs()
function bait__checker__Checker_check_attributes(c, node) {
	for (let _t9 = 0; _t9 < node.attrs.length; _t9++) {
		const attr = array_get(node.attrs, _t9)
		if (!map_contains(bait__checker__KNOWN_ATTRS, attr.name)) {
		...
		if (val_req == bait__checker__AttrValue.required && attr.value.length == 0) {
...
function bait__checker__Checker({ parsed_files = new array({ data: [], length: 0 }), table = new bait__ast__Table({}), scope = new bait__ast__Scope({}), path = from_js_string(""), pkg = from_js_string(""), is_js_file = false, had_error = false, cur_fun = new bait__ast__FunDecl({}), expected_type = 0, is_lhs_assign = false }) {
...
function bait__checker__check_files(files, table) {
	...
	for (let _t10 = 0; _t10 < c.parsed_files.length; _t10++) {
		const f = array_get(c.parsed_files, _t10)
		bait__checker__Checker_check(c, f)
	}
	bait__checker__Checker_check_redefined_functions(c)
	return !c.had_error
}
...
function bait__checker__Checker_generic_error(c, msg) {
	bait__errors__generic_error(msg)
	c.had_error = true
}
...
function bait__checker__Checker_match_expr(c, node) {
	...
	for (let _t11 = 0; _t11 < node.branches.length; _t11++) {
		const branch = array_get(node.branches, _t11)
		bait__checker__Checker_open_scope(c)
		for (let _t12 = 0; _t12 < branch.exprs.length; _t12++) {
			const e = array_get(branch.exprs, _t12)
			if (node.is_sumtype) {
				const expr = e
...
function bait__checker__Checker_string_inter_literal(c, node) {
	for (let _t13 = 0; _t13 < node.exprs.length; _t13++) {
		const e = array_get(node.exprs, _t13)
		const typ = bait__checker__Checker_expr(c, e)
...
function bait__checker__Checker_struct_init(c, node) {
	...
	node.name = sym.name
	for (let _t14 = 0; _t14 < node.fields.length; _t14++) {
		const field = array_get(node.fields, _t14)
...
function bait__checker__Checker_fun_decl(c, node) {
	c.cur_fun = node
	bait__checker__Checker_check_attributes(c, node)
	bait__checker__Checker_open_scope(c)
...
function bait__checker__FunRedefinition({ path = from_js_string(""), pos = new bait__token__Pos({}), name = from_js_string(""), signature = from_js_string("") }) {
	this.path = path
	this.pos = pos
	this.name = name
	this.signature = signature
}
bait__checker__FunRedefinition.prototype = {
	toString() {
		return `bait__checker__FunRedefinition{
    path: ${this.path.toString()}
    pos: ${this.pos.toString()}
    name: ${this.name.toString()}
...
function bait__checker__Checker_check_redefined_functions(c) {
	let unique_redefs = new array({ data: [], length: 0 })
	for (let _t16 = 0; _t16 < c.table.redefined_funs.length; _t16++) {
		const name = array_get(c.table.redefined_funs, _t16)
		if (!array_string_contains(unique_redefs, name)) {
			array_push(unique_redefs, name)
		}
	}
	for (let _t17 = 0; _t17 < unique_redefs.length; _t17++) {
		const name = array_get(unique_redefs, _t17)
		let redefinitions = new array({ data: [], length: 0 })
		for (let _t18 = 0; _t18 < c.parsed_files.length; _t18++) {
			const file = array_get(c.parsed_files, _t18)
			for (let _t19 = 0; _t19 < file.stmts.length; _t19++) {
				const stmt = array_get(file.stmts, _t19)
				if (stmt instanceof bait__ast__FunDecl && string_eq(stmt.name, name)) {
					array_push(redefinitions, new bait__checker__FunRedefinition({ path: file.path, pos: stmt.pos, name: name, signature: bait__checker__Checker_fun_signature(c, stmt) }))
				}
			}
		}
		bait__checker__Checker_generic_error(c, from_js_string(`redfinition of function "${name.str}"`))
		for (let _t20 = 0; _t20 < redefinitions.length; _t20++) {
			const redef = array_get(redefinitions, _t20)
			bait__errors__notice(from_js_string("conflict"), redef.path, redef.pos, redef.signature)
		}
	}
}
...
function bait__checker__Checker_method_call(c, node) {
	...
	if (left_sym.kind == bait__ast__TypeKind.array && array_string_contains(new array({ data: [from_js_string("push"), from_js_string("index"), from_js_string("last_index"), from_js_string("contains")], length: 4 }), node.name)) {
	...
	if (left_sym.kind == bait__ast__TypeKind.array && string_eq(node.name, from_js_string("concat"))) {
		const arg = array_get(node.args, 0)
...
function bait__checker__Checker_check_attributes_on_call(c, call, def) {
	let is_deprecated = false
	let depr_attr = new bait__ast__Attribute({})
	let depr_date_attr = new bait__ast__Attribute({})
	for (let _t22 = 0; _t22 < def.attrs.length; _t22++) {
		const attr = array_get(def.attrs, _t22)
	...
	if (is_deprecated) {
		...
		bait__errors__notice(from_js_string("note"), c.path, call.pos, depr_message)
	}
}
...
function bait__checker__Checker_return_stmt(c, node) {
	if (node.expr instanceof bait__ast__EmptyExpr) {
		if (c.cur_fun.return_type != bait__ast__VOID_TYPE) {
...
function bait__checker__Checker_check_types(c, got, expected) {
	...
	if (got == bait__ast__ANY_TYPE || expected == bait__ast__ANY_TYPE) {
		return true
	}
	...
	if (exp_sym.kind == bait__ast__TypeKind.array && got_sym.kind == bait__ast__TypeKind.array) {
		...
		if (got_info.elem_type == bait__ast__ANY_TYPE) {
			return true
		}
		const exp_info = exp_sym.info
		if (exp_info.elem_type == bait__ast__ANY_TYPE) {
			return true
		}
	}
...
const bait__util__VERSION = from_js_string(`0.0.3-dev ${from_js_string("5adc5f4").str}`)
...
function bait__gen__js__Gen_call_expr(g, node) {
	if (node.is_method && node.lang != bait__ast__Language.bait) {
...
function bait__gen__js__Gen_infix_expr(g, node) {
	...
	if (node.left_type == bait__ast__STRING_TYPE) {
	...
	if (node.left_type == bait__ast__U8_TYPE) {
		if (node.op == bait__token__TokenKind.plus) {
...
function bait__gen__js__Gen_match_expr(g, node) {
	...
	if (node.cond_type == bait__ast__STRING_TYPE) {
		bait__gen__js__Gen_write(g, from_js_string(".str"))
	} else if (node.cond_type == bait__ast__U8_TYPE) {
		bait__gen__js__Gen_write(g, from_js_string(".val"))
	}
	bait__gen__js__Gen_writeln(g, from_js_string(") {"))
	g.indent += 1
	for (let _t24 = 0; _t24 < node.branches.length; _t24++) {
		const b = array_get(node.branches, _t24)
		if (b.exprs.length == 0) {
...
function bait__gen__js__gen(files, table, pref) {
	...
	for (let _t25 = 0; _t25 < files.length; _t25++) {
		const file = array_get(files, _t25)
		g.path = file.path
	...
	return string_add(string_add(string_add(g.type_defs_out, g.global_out), from_js_string("\n")), g.out)
}
...
function bait__gen__js__Gen_gen_test_main(g) {
	...
	g.indent += 1
	const _t26 = map_keys(g.table.fun_decls)
	for (let _t27 = 0; _t27 < _t26.length; _t27++) {
		const key = array_get(_t26, _t27)
...
function bait__gen__js__Gen_stmts(g, stmts) {
	g.indent += 1
	for (let _t28 = 0; _t28 < stmts.length; _t28++) {
		const stmt = array_get(stmts, _t28)
...
function bait__gen__js__Gen_assert_stmt(g, node) {
	...
	if (node.expr instanceof bait__ast__InfixExpr) {
		const expr = node.expr
		bait__gen__js__Gen_write(g, from_js_string(`\tTestRunner_set_assert_info(test_runner, ${i32_str(node.pos.line)}, from_js_string("assert `))
...
function bait__gen__js__Gen_enum_decl(g, node) {
	...
	for (let i = 0; i < node.variants.length; i++) {
		...
		bait__gen__js__Gen_writeln(g, from_js_string(`${variant.str}: ${i32_str(i)},`))
	}
	g.indent -= 1
	bait__gen__js__Gen_writeln(g, from_js_string("}"))
}
...
function bait__gen__js__Gen_struct_decl(g, node) {
	...
	bait__gen__js__Gen_writeln(g, from_js_string("}) {"))
	g.indent += 1
	for (let _t29 = 0; _t29 < node.fields.length; _t29++) {
		const field = array_get(node.fields, _t29)
	...
	for (let _t30 = 0; _t30 < node.fields.length; _t30++) {
		const field = array_get(node.fields, _t30)
		bait__gen__js__Gen_write(g, from_js_string(`    ${field.name.str}: \$`))
...
function bait__builder__Builder_collect_bait_files(b, dir) {
	...
	for (let _t31 = 0; _t31 < all_files.length; _t31++) {
		const f = array_get(all_files, _t31)
		...
		if (b.prefs.backend == bait__preference__Backend.js && string_ends_with(f, from_js_string(".c.bt"))) {
			continue
		}
		if (b.prefs.backend == bait__preference__Backend.c && string_ends_with(f, from_js_string(".js.bt"))) {
			continue
		}
...
function bait__builder__compile(prefs) {
	...
	let files = new array({ data: [], length: 0 })
	for (let _t32 = 0; _t32 < paths.length; _t32++) {
		const p = array_get(paths, _t32)
	...
	for (let i = 0; i < files.length; i += 1) {
		...
		for (let _t33 = 0; _t33 < f.imports.length; _t33++) {
			const imp = array_get(f.imports, _t33)
			...
			for (let _t34 = 0; _t34 < imp_paths.length; _t34++) {
				const p = array_get(imp_paths, _t34)
				array_push(paths, p)
				array_push(files, bait__builder__Builder_parse_source_file(b, p))
			}
		}
	...
	for (let _t35 = 0; _t35 < files.length; _t35++) {
		...
		for (let _t36 = 0; _t36 < f.imports.length; _t36++) {
			const imp = array_get(f.imports, _t36)
	...
	for (let _t37 = 0; _t37 < pkg_order.length; _t37++) {
		const pkg = array_get(pkg_order, _t37)
		for (let _t38 = 0; _t38 < files.length; _t38++) {
			const f = array_get(files, _t38)
			if (string_eq(f.pkg_decl.full_name, pkg)) {
				array_push(sorted_files, f)
			}
		}
	}
	if (!bait__checker__check_files(sorted_files, b.table)) {
		return 1
	}
	if (b.prefs.backend != bait__preference__Backend.js) {
...
function bait__builder__order_pkgs(ordered, pkg, deps) {
	for (let _t39 = 0; _t39 < map_get_set(deps, pkg, new array({ data: [], length: 0 })).length; _t39++) {
		const d = array_get(map_get_set(deps, pkg, new array({ data: [], length: 0 })), _t39)
...
function bait__builder__run_tests(prefs) {
	...
	for (let _t40 = 0; _t40 < prefs.args.length; _t40++) {
		const a = array_get(prefs.args, _t40)

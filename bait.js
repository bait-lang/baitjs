...
function bait__checker__Checker_struct_init(c, node) {
	...
	node.name = sym.name
	for (let _t775 = 0; _t775 < node.fields.length; _t775++) {
		const field = Array_get(node.fields, _t775)
...
function bait__checker__Checker_check_init_field_values(c, init, info) {
	for (let _t778 = 0; _t778 < info.fields.length; _t778++) {
		const def_field = Array_get(info.fields, _t778)
		...
		if (!is_required) {
			for (let _t780 = 0; _t780 < def_field.attrs.length; _t780++) {
				const attr = Array_get(def_field.attrs, _t780)
		...
		for (let _t783 = 0; _t783 < init.fields.length; _t783++) {
			const inited = Array_get(init.fields, _t783)
...
function bait__checker__Checker_does_type_exist(c, sym, pos) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.sum_type)) {
		const info = sym.info
		for (let _t808 = 0; _t808 < info.variants.length; _t808++) {
			const variant = Array_get(info.variants, _t808)
	...
	if (eq(sym.kind, bait__ast__TypeKind.fun_)) {
		const info = sym.info
		for (let _t810 = 0; _t810 < info.param_types.length; _t810++) {
			const param_type = Array_get(info.param_types, _t810)
...
function bait__util__escape__linebreak(s) {
	let _t816 = undefined
	if (string_eq(os__platform(), from_js_string("windows"))) {
		_t816 = string_replace(s, from_js_string("\r\n"), from_js_string("\\r\\n"))
	} else {
		_t816 = string_replace(s, from_js_string("\n"), from_js_string("\\n"))
	}
	return _t816
...
let _t817 = undefined
if (string_eq(os__platform(), from_js_string("windows"))) {
	_t817 = from_js_string("\\r\\n")
} else {
	_t817 = from_js_string("\\n")
}
const bait__gen__js__LB = _t817
...
function bait__gen__js__Gen_generate_str_fun(g, typ) {
	...
	let s = "${sym.name.str}{"\n`))
		...
		for (let _t822 = 0; _t822 < info.fields.length; _t822++) {
			const field = Array_get(info.fields, _t822)
			if (eq(typ, field.typ)) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.sum_type)) {
		const info = sym.info
		g.fun_decls_out = string_add(g.fun_decls_out, from_js_string(`function ${name.str}(it, indent) {\n`))
		for (let _t826 = 0; _t826 < info.variants.length; _t826++) {
			const bait_var = Array_get(info.variants, _t826)
...
`))
	...
	switch(it) {\n`))
		const info = sym.info
		for (let _t827 = 0; _t827 < info.vals.length; _t827++) {
			const val = Array_get(info.vals, _t827)
...
function bait__gen__js__Gen_get_comptime_val(g, kind, pos) {
	...
	if (eq(kind, bait__token__ComptimeVar.pkg)) {
		...
		_t829 = string_replace(os__abs_path(g.path), from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.file)) {
		_t829 = string_replace(g.path, from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.dir)) {
		_t829 = os__dir(bait__gen__js__Gen_get_comptime_val(g, bait__token__ComptimeVar.abs_file, pos))
	} else if (eq(kind, bait__token__ComptimeVar.line)) {
		_t829 = i32_str(pos.line)
		...
		_t829 = from_js_string(`${file.str}:${line.str}`)
	} else if (eq(kind, bait__token__ComptimeVar.fun_)) {
		_t829 = g.cur_fun.name
	} else if (eq(kind, bait__token__ComptimeVar.baitexe)) {
		_t829 = bait__gen__js__Gen_comptime_baitexe(g)
	} else if (eq(kind, bait__token__ComptimeVar.baitdir)) {
		_t829 = bait__gen__js__Gen_comptime_baitdir(g)
	} else if (eq(kind, bait__token__ComptimeVar.baithash)) {
		_t829 = bait__gen__js__Gen_comptime_baithash(g)
	} else if (eq(kind, bait__token__ComptimeVar.unknown)) {
		_t829 = panic(from_js_string("this should never happen"))
	}
	return _t829
}
...
function bait__gen__js__Gen_fun_decl(g, node) {
	...
	if (i32(node.generic_names.length > 0) && eq(g.cur_concrete_types.length, 0)) {
		...
		for (let _t862 = 0; _t862 < gtypes.length; _t862++) {
			const conc_types = Array_get(gtypes, _t862)
...
function bait__gen__js__gen(files, table, pref) {
	...
	for (let _t888 = 0; _t888 < files.length; _t888++) {
		const file = Array_get(files, _t888)
		g.path = file.path
		g.pkg = file.pkg_name
	...
	for (let _t890 = 0; _t890 < g.table.needed_str_funs.length; _t890++) {
		const typ = Array_get(g.table.needed_str_funs, _t890)
...
function bait__gen__js__Gen_process_imports(g, imports) {
	for (let _t890 = 0; _t890 < imports.length; _t890++) {
		const imp = Array_get(imports, _t890)
...
function bait__gen__js__Gen_headers(g) {
	...
	const _t892 = Map_keys(g.foreign_imports)
	for (let _t893 = 0; _t893 < _t892.length; _t893++) {
		const alias = Array_get(_t892, _t893)
...
function bait__gen__js__Gen_cut_before_any(g, chars) {
	for (let i = i32(g.out.length - 1); i32(i >= 0); i -= 1) {
		for (let _t895 = 0; _t895 < chars.length; _t895++) {
			const c = string_get(chars, _t895)
...
function bait__gen__js__Gen_gen_test_main(g) {
	...
	const _t898 = Map_keys(g.table.fun_decls)
	for (let _t899 = 0; _t899 < _t898.length; _t899++) {
		const key = Array_get(_t898, _t899)
...
function bait__gen__js__Gen_get_concrete_name(g, name, concrete_types) {
	let full_name = name
	for (let _t905 = 0; _t905 < concrete_types.length; _t905++) {
		const t = Array_get(concrete_types, _t905)
...
function bait__gen__js__Gen_stmts(g, stmts) {
	bait__gen__js__Gen_save_stmt_offset(g)
	g.indent += 1
	for (let _t907 = 0; _t907 < stmts.length; _t907++) {
		const stmt = Array_get(stmts, _t907)
...
function bait__gen__js__Gen_enum_decl(g, node) {
	...
	g.indent += 1
	for (let _t920 = 0; _t920 < node.fields.length; _t920++) {
		const field = Array_get(node.fields, _t920)
...
let _t938 = undefined
if (string_eq(os__platform(), from_js_string("windows"))) {
	_t938 = from_js_string("\\r\\n")
} else {
	_t938 = from_js_string("\\n")
}
const bait__gen__c__LB = _t938
...
function bait__gen__c__Gen_generate_str_fun(g, typ) {
	...
	strings__Builder_write(&b, from_c_string("${sym.name.str}{"));\n`))
		...
		for (let _t942 = 0; _t942 < info.fields.length; _t942++) {
			const field = Array_get(info.fields, _t942)
			if (eq(typ, field.typ)) {
...
function bait__gen__c__Gen_process_imports(g, imports) {
	for (let _t945 = 0; _t945 < imports.length; _t945++) {
		const imp = Array_get(imports, _t945)
...
function bait__gen__c__Gen_headers(g) {
	...
	for (let _t946 = 0; _t946 < g.foreign_imports.length; _t946++) {
		const name = Array_get(g.foreign_imports, _t946)
...
function bait__gen__c__Gen_gen_test_main(g) {
	...
	const _t948 = Map_keys(g.table.fun_decls)
	for (let _t949 = 0; _t949 < _t948.length; _t949++) {
		const key = Array_get(_t948, _t949)
		const func = Map_get_set(g.table.fun_decls, key, new bait__ast__FunDecl({}))
		if (func.is_test) {
			nr_test_funs += 1
...
function bait__gen__c__Gen_get_concrete_name(g, name, concrete_types) {
	let full_name = name
	for (let _t952 = 0; _t952 < concrete_types.length; _t952++) {
		const t = Array_get(concrete_types, _t952)
		full_name = string_add(full_name, string_add(from_js_string("_"), bait__ast__Table_get_sym(g.table, t).name))
	}
...
function bait__gen__c__Gen_get_comptime_val(g, kind, pos) {
	...
	if (eq(kind, bait__token__ComptimeVar.pkg)) {
		...
		_t957 = string_replace(os__abs_path(g.path), from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.file)) {
		_t957 = string_replace(g.path, from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.dir)) {
		_t957 = os__dir(bait__gen__c__Gen_get_comptime_val(g, bait__token__ComptimeVar.abs_file, pos))
	} else if (eq(kind, bait__token__ComptimeVar.line)) {
		_t957 = i32_str(pos.line)
	} else if (eq(kind, bait__token__ComptimeVar.file_line)) {
		...
		_t957 = from_js_string(`${file.str}:${line.str}`)
	} else if (eq(kind, bait__token__ComptimeVar.fun_)) {
		_t957 = g.cur_fun.name
	} else if (eq(kind, bait__token__ComptimeVar.baitexe)) {
		_t957 = bait__gen__c__Gen_comptime_baitexe(g)
	} else if (eq(kind, bait__token__ComptimeVar.baitdir)) {
		_t957 = bait__gen__c__Gen_comptime_baitdir(g)
	} else if (eq(kind, bait__token__ComptimeVar.baithash)) {
		_t957 = bait__gen__c__Gen_comptime_baithash(g)
	} else if (eq(kind, bait__token__ComptimeVar.unknown)) {
		_t957 = panic(from_js_string("this should never happen"))
	}
	return _t957
}
...
function bait__gen__c__Gen_gen_equality_funs(g) {
	for (let _t960 = 0; _t960 < g.table.needed_equality_funs.length; _t960++) {
		const typ = Array_get(g.table.needed_equality_funs, _t960)
...
function bait__gen__c__Gen_array_init(g, node) {
	...
	g.indent += 1
	for (let _t968 = 0; _t968 < node.exprs.length; _t968++) {
		const expr = Array_get(node.exprs, _t968)
		bait__gen__c__Gen_expr(g, expr)
		bait__gen__c__Gen_write(g, from_js_string(", "))
	}
	g.indent -= 1
...
function bait__gen__c__Gen_fun_decl(g, node) {
	...
	if (i32(node.generic_names.length > 0) && eq(g.cur_concrete_types.length, 0)) {
		const gtypes = Map_get_set(g.table.generic_fun_types, bait__ast__FunDecl_key(node), new bait_Array({ data: [], length: 0 }))
		for (let _t987 = 0; _t987 < gtypes.length; _t987++) {
			const conc_types = Array_get(gtypes, _t987)
			for (let i = 0; i < node.generic_names.length; i++) {
				const gn = Array_get(node.generic_names, i)
				Map_set(g.cur_concrete_types, gn, Array_get(conc_types, i))
			}
...
function bait__gen__c__Gen_stmts(g, stmts) {
	bait__gen__c__Gen_save_stmt_offset(g)
	g.indent += 1
	for (let _t1014 = 0; _t1014 < stmts.length; _t1014++) {
		const stmt = Array_get(stmts, _t1014)
...
function bait__gen__c__Gen_typ(g, typ) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.enum_)) {
		_t1026 = from_js_string("enum ")
	} else {
		_t1026 = from_js_string("")
	}
	const prefix = _t1026
...
function bait__gen__c__Gen_write_types(g) {
	for (let _t1026 = 0; _t1026 < g.table.type_symbols.length; _t1026++) {
		const sym = Array_get(g.table.type_symbols, _t1026)
		...
		if (sym.info instanceof bait__ast__StructInfo) {
			...
			for (let _t1028 = 0; _t1028 < info.fields.length; _t1028++) {
				const field = Array_get(info.fields, _t1028)
			...
			for (let _t1030 = 0; _t1030 < info.vals.length; _t1030++) {
				const val = Array_get(info.vals, _t1030)
...
function bait__transformer__Transformer_transform_files(t, files) {
	for (let _t1030 = 0; _t1030 < files.length; _t1030++) {
		let file = Array_get(files, _t1030)
...
function bait__transformer__Transformer_if_match(t, node) {
	for (let _t1035 = 0; _t1035 < node.branches.length; _t1035++) {
		let branch = Array_get(node.branches, _t1035)
...
function bait__transformer__Transformer_call_expr(t, node) {
	bait__transformer__Transformer_expr(t, node.left)
	for (let _t1035 = 0; _t1035 < node.args.length; _t1035++) {
		const arg = Array_get(node.args, _t1035)
...
function bait__builder__Builder_bait_files_in_dir(b, dir) {
	...
	for (let _t1039 = 0; _t1039 < all_files.length; _t1039++) {
		const f = Array_get(all_files, _t1039)
...
function bait__builder__Builder_get_builtin_files(b) {
	let _t1042 = bait__builder__Builder_bait_files_in_dir(b, bait__builder__Builder_resolve_import(b, from_js_string(""), from_js_string("builtin")))
	if (_t1042.is_error) {
		const err = _t1042.msg
		_t1042.data = panic(err)
	}
	return _t1042.data
}
...
function bait__builder__Builder_collect_user_files(b, path) {
	...
	let _t1044 = bait__builder__Builder_bait_files_in_dir(b, path)
	if (_t1044.is_error) {
		return _t1044
	}
	return new Result({ data: _t1044.data })
}
...
function bait__builder__compile(prefs) {
	...
	let _t1045 = bait__builder__Builder_collect_user_files(b, prefs.command)
	if (_t1045.is_error) {
		const err = _t1045.msg
		bait__errors__generic_error(err)
		return 1
	}
	Array_push_many(paths, _t1045.data)
	bait__util__timers__start(from_js_string("PARSE"))
	let ast_files = new bait_Array({ data: [], length: 0 })
	for (let _t1045 = 0; _t1045 < paths.length; _t1045++) {
		const p = Array_get(paths, _t1045)
	...
	for (let i = 0; i32(i < ast_files.length); i += 1) {
		...
		for (let _t1045 = 0; _t1045 < file.imports.length; _t1045++) {
			const imp = Array_get(file.imports, _t1045)
			...
			let _t1049 = bait__builder__Builder_bait_files_in_dir(b, import_dir)
			if (_t1049.is_error) {
				const err = _t1049.msg
				bait__errors__generic_error(from_js_string(`package \`${imp.name.str}\` contains no .bt files`))
				continue
				_t1049.data = new bait_Array({ data: [], length: 0 })
			}
			const imp_paths = _t1049.data
			let nr_newly_parsed = 0
			for (let _t1049 = 0; _t1049 < imp_paths.length; _t1049++) {
				const p = Array_get(imp_paths, _t1049)
	...
	for (let _t1052 = 0; _t1052 < ast_files.length; _t1052++) {
		...
		for (let _t1052 = 0; _t1052 < f.imports.length; _t1052++) {
			const imp = Array_get(f.imports, _t1052)
			if (!eq(imp.lang, bait__ast__Language.bait)) {
				continue
			}
	...
	for (let _t1053 = 0; _t1053 < pkg_order.length; _t1053++) {
		const pkg = Array_get(pkg_order, _t1053)
		for (let _t1053 = 0; _t1053 < ast_files.length; _t1053++) {
			const f = Array_get(ast_files, _t1053)
...
function bait__builder__Builder_print_infos(b, infos) {
	...
	for (let _t1066 = 0; _t1066 < infos.length; _t1066++) {
		const info = Array_get(infos, _t1066)
		bait__errors__Message_print(info)
	}
}
...
function bait__builder__Builder_print_errors_and_warnings(b, parser_errs) {
	let nr_warns = 0
	let nr_errors = 0
	for (let _t1066 = 0; _t1066 < b.parsed_files.length; _t1066++) {
		const f = Array_get(b.parsed_files, _t1066)
		nr_warns += f.warnings.length
		nr_errors += f.errors.length
		bait__builder__Builder_print_infos(b, f.infos)
		if (b.prefs.warn_is_error) {
			for (let _t1067 = 0; _t1067 < f.warnings.length; _t1067++) {
				const warn = Array_get(f.warnings, _t1067)
		...
		for (let _t1067 = 0; _t1067 < f.errors.length; _t1067++) {
			const err = Array_get(f.errors, _t1067)
	...
	for (let _t1068 = 0; _t1068 < b.checker.errors.length; _t1068++) {
		const err = Array_get(b.checker.errors, _t1068)
		bait__errors__Message_print(err)
	}
...
function bait__builder__order_pkgs(ordered, pkg, deps, looked) {
	...
	for (let _t1073 = 0; _t1073 < Map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })).length; _t1073++) {
		const d = Array_get(Map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })), _t1073)
...
function bait__builder__run_tests(prefs) {
	...
	for (let _t1076 = 0; _t1076 < prefs.args.length; _t1076++) {
		const a = Array_get(prefs.args, _t1076)
...
const bait__util__FULL_VERSION = from_js_string(`${bait__util__VERSION.str} ${from_js_string("5ab5644").str}`)

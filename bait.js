...
function bait__checker__Checker_fun_params(c, params) {
	for (let _t288 = 0; _t288 < params.length; _t288++) {
		const p = Array_get(params, _t288)
...
function bait__checker__Checker_check_main_fun(c, stmts) {
	for (let _t291 = 0; _t291 < stmts.length; _t291++) {
		const stmt = Array_get(stmts, _t291)
...
function bait__checker__has_toplevel_return(stmts) {
	for (let _t327 = 0; _t327 < stmts.length; _t327++) {
		const stmt = Array_get(stmts, _t327)
...
function bait__checker__Checker_match_expr(c, node, is_expr) {
	...
	for (let _t337 = 0; _t337 < node.branches.length; _t337++) {
		const branch = Array_get(node.branches, _t337)
		bait__checker__Checker_open_scope(c)
		for (let _t338 = 0; _t338 < branch.exprs.length; _t338++) {
			const e = Array_get(branch.exprs, _t338)
			if (node.is_sumtype) {
				const expr = e
...
function bait__checker__Checker_stmts(c, stmts) {
	for (let _t353 = 0; _t353 < stmts.length; _t353++) {
		const stmt = Array_get(stmts, _t353)
...
function bait__checker__Checker_struct_init(c, node) {
	...
	node.name = sym.name
	for (let _t381 = 0; _t381 < node.fields.length; _t381++) {
		const field = Array_get(node.fields, _t381)
...
function bait__checker__Checker_check_init_field_values(c, init, info) {
	for (let _t385 = 0; _t385 < info.fields.length; _t385++) {
		const def_field = Array_get(info.fields, _t385)
		const def_sym = bait__ast__Table_get_sym(c.table, def_field.typ)
		let is_required = eq(def_sym.kind, bait__ast__TypeKind.sum_type)
		for (let _t386 = 0; _t386 < def_field.attrs.length; _t386++) {
			const attr = Array_get(def_field.attrs, _t386)
		...
		for (let _t390 = 0; _t390 < init.fields.length; _t390++) {
			const inited = Array_get(init.fields, _t390)
...
const bait__util__FULL_VERSION = from_js_string(`${bait__util__VERSION.str} ${from_js_string("4687d00").str}`)
...
function bait__gen__js__Gen_fun_decl(g, node) {
	...
	if (i32(node.generic_names.length > 0) && eq(g.cur_concrete_types.length, 0)) {
		for (let _t444 = 0; _t444 < node.concrete_types.length; _t444++) {
			const conc_types = Array_get(node.concrete_types, _t444)
...
function bait__gen__js__Gen_generic_fun_name(g, name, concrete_types) {
	let full_name = name
	for (let _t458 = 0; _t458 < concrete_types.length; _t458++) {
		const t = Array_get(concrete_types, _t458)
...
function bait__gen__js__Gen_match_expr(g, node, is_expr) {
	...
	bait__gen__js__Gen_writeln(g, from_js_string(") {"))
	g.indent += 1
	for (let _t467 = 0; _t467 < node.branches.length; _t467++) {
		const b = Array_get(node.branches, _t467)
...
function bait__gen__js__gen(files, table, pref) {
	...
	for (let _t475 = 0; _t475 < files.length; _t475++) {
		const file = Array_get(files, _t475)
		g.path = file.path
	...
	for (let _t478 = 0; _t478 < g.table.needed_str_funs.length; _t478++) {
		const typ = Array_get(g.table.needed_str_funs, _t478)
...
function bait__gen__js__Gen_process_imports(g, imports) {
	for (let _t479 = 0; _t479 < imports.length; _t479++) {
		const imp = Array_get(imports, _t479)
...
function bait__gen__js__Gen_headers(g) {
	...
	const _t481 = map_keys(g.foreign_imports)
	for (let _t482 = 0; _t482 < _t481.length; _t482++) {
		const alias = Array_get(_t481, _t482)
...
function bait__gen__js__Gen_gen_test_main(g) {
	...
	g.indent += 1
	const _t485 = map_keys(g.table.fun_decls)
	for (let _t486 = 0; _t486 < _t485.length; _t486++) {
		const key = Array_get(_t485, _t486)
...
function bait__gen__js__Gen_stmts(g, stmts) {
	g.indent += 1
	for (let _t493 = 0; _t493 < stmts.length; _t493++) {
		const stmt = Array_get(stmts, _t493)
...
function bait__gen__js__Gen_enum_decl(g, node) {
	...
	g.indent += 1
	for (let _t507 = 0; _t507 < node.fields.length; _t507++) {
		const field = Array_get(node.fields, _t507)
...
function bait__gen__js__Gen_generate_str_fun(g, typ) {
	...
	let s = "${sym.name.str}{"\n`))
		...
		for (let _t521 = 0; _t521 < info.fields.length; _t521++) {
			const field = Array_get(info.fields, _t521)
			if (eq(typ, field.typ)) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.sum_type)) {
		const info = sym.info
		g.fun_decls_out = string_add(g.fun_decls_out, from_js_string(`function ${name.str}(it, indent) {\n`))
		for (let _t526 = 0; _t526 < info.variants.length; _t526++) {
			const bait_var = Array_get(info.variants, _t526)
...
`))
	...
	switch(it) {\n`))
		const info = sym.info
		for (let _t528 = 0; _t528 < info.vals.length; _t528++) {
			const val = Array_get(info.vals, _t528)
...
function bait__gen__js__Gen_struct_decl(g, node) {
	...
	bait__gen__js__Gen_writeln(g, from_js_string("}) {"))
	g.indent += 1
	for (let _t533 = 0; _t533 < node.fields.length; _t533++) {
		const field = Array_get(node.fields, _t533)
...
function bait__gen__c__gen(files, table, pref) {
	...
	for (let _t536 = 0; _t536 < files.length; _t536++) {
		const file = Array_get(files, _t536)
		g.path = file.path
		g.pkg = file.pkg_decl.full_name
...
function bait__gen__c__Gen_process_imports(g, imports) {
	for (let _t538 = 0; _t538 < imports.length; _t538++) {
		const imp = Array_get(imports, _t538)
...
function bait__gen__c__Gen_headers(g) {
	...
	for (let _t540 = 0; _t540 < g.foreign_imports.length; _t540++) {
		const name = Array_get(g.foreign_imports, _t540)
...
function bait__gen__c__Gen_gen_equality_funs(g) {
	for (let _t543 = 0; _t543 < g.table.needed_equality_funs.length; _t543++) {
		const typ = Array_get(g.table.needed_equality_funs, _t543)
...
function bait__gen__c__Gen_array_init(g, node) {
	...
	g.indent += 1
	for (let _t550 = 0; _t550 < node.exprs.length; _t550++) {
		const expr = Array_get(node.exprs, _t550)
		bait__gen__c__Gen_expr(g, expr)
		bait__gen__c__Gen_write(g, from_js_string(", "))
	}
	g.indent -= 1
...
function bait__gen__c__Gen_fun_decl(g, node) {
	...
	if (i32(node.generic_names.length > 0) && eq(g.cur_concrete_types.length, 0)) {
		for (let _t572 = 0; _t572 < node.concrete_types.length; _t572++) {
			const conc_types = Array_get(node.concrete_types, _t572)
			for (let i = 0; i < node.generic_names.length; i++) {
				const gn = Array_get(node.generic_names, i)
				map_set(g.cur_concrete_types, gn, Array_get(conc_types, i))
			}
...
function bait__gen__c__Gen_generic_fun_name(g, name, concrete_types) {
	let full_name = name
	for (let _t589 = 0; _t589 < concrete_types.length; _t589++) {
		const t = Array_get(concrete_types, _t589)
		full_name = string_add(full_name, string_add(from_js_string("_"), bait__ast__Table_get_sym(g.table, t).name))
	}
	return full_name
}
...
function bait__gen__c__Gen_write_types(g) {
	for (let _t605 = 0; _t605 < g.table.type_symbols.length; _t605++) {
		const sym = Array_get(g.table.type_symbols, _t605)
		...
		if (sym.info instanceof bait__ast__StructInfo) {
			...
			for (let _t608 = 0; _t608 < info.fields.length; _t608++) {
				const field = Array_get(info.fields, _t608)
...
function bait__builder__Builder_collect_bait_files(b, dir) {
	...
	for (let _t631 = 0; _t631 < all_files.length; _t631++) {
		const f = Array_get(all_files, _t631)
...
function bait__builder__compile(prefs) {
	...
	let files = new bait_Array({ data: [], length: 0 })
	for (let _t635 = 0; _t635 < paths.length; _t635++) {
		const p = Array_get(paths, _t635)
	...
	for (let i = 0; i32(i < files.length); i += 1) {
		...
		for (let _t636 = 0; _t636 < f.imports.length; _t636++) {
			const imp = Array_get(f.imports, _t636)
			...
			for (let _t639 = 0; _t639 < imp_paths.length; _t639++) {
				const p = Array_get(imp_paths, _t639)
				Array_push(paths, p)
	...
	for (let _t641 = 0; _t641 < files.length; _t641++) {
		const f = Array_get(files, _t641)
		const pkg_name = f.pkg_decl.full_name
		for (let _t642 = 0; _t642 < f.imports.length; _t642++) {
			const imp = Array_get(f.imports, _t642)
			if (!eq(imp.lang, bait__ast__Language.bait)) {
				continue
			}
	...
	for (let _t644 = 0; _t644 < pkg_order.length; _t644++) {
		const pkg = Array_get(pkg_order, _t644)
		for (let _t645 = 0; _t645 < files.length; _t645++) {
			const f = Array_get(files, _t645)
...
function bait__builder__Builder_print_errors_and_warnings(b) {
	let nr_warns = 0
	let nr_errors = 0
	for (let _t656 = 0; _t656 < b.parsed_files.length; _t656++) {
		const f = Array_get(b.parsed_files, _t656)
		nr_warns += f.warnings.length
		nr_errors += f.errors.length
		if (!b.prefs.hide_warnings) {
			for (let _t658 = 0; _t658 < f.infos.length; _t658++) {
				const info = Array_get(f.infos, _t658)
				bait__errors__Message_print(info)
			}
		}
		if (b.prefs.warn_is_error) {
			for (let _t660 = 0; _t660 < f.warnings.length; _t660++) {
				const warn = Array_get(f.warnings, _t660)
				bait__errors__error(warn.path, warn.pos, warn.msg)
			}
		} else if (!b.prefs.hide_warnings) {
			for (let _t661 = 0; _t661 < f.warnings.length; _t661++) {
				const warn = Array_get(f.warnings, _t661)
				bait__errors__Message_print(warn)
			}
		}
		for (let _t662 = 0; _t662 < f.errors.length; _t662++) {
			const err = Array_get(f.errors, _t662)
			bait__errors__Message_print(err)
		}
	}
	nr_errors += b.checker.errors.length
	for (let _t663 = 0; _t663 < b.checker.errors.length; _t663++) {
		const err = Array_get(b.checker.errors, _t663)
		bait__errors__Message_print(err)
	}
...
function bait__builder__order_pkgs(ordered, pkg, deps, looked) {
	...
	for (let _t666 = 0; _t666 < map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })).length; _t666++) {
		const d = Array_get(map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })), _t666)
...
function bait__builder__Builder_check_redefined_functions(b) {
	...
	for (let _t671 = 0; _t671 < b.table.redefined_funs.length; _t671++) {
		const name = Array_get(b.table.redefined_funs, _t671)
		if (!Array_contains(unique_redefs, name)) {
			Array_push(unique_redefs, name)
		}
	}
	for (let _t673 = 0; _t673 < unique_redefs.length; _t673++) {
		const name = Array_get(unique_redefs, _t673)
		let redefinitions = new bait_Array({ data: [], length: 0 })
		for (let _t674 = 0; _t674 < b.parsed_files.length; _t674++) {
			const file = Array_get(b.parsed_files, _t674)
			for (let _t675 = 0; _t675 < file.stmts.length; _t675++) {
				const stmt = Array_get(file.stmts, _t675)
		...
		for (let _t677 = 0; _t677 < redefinitions.length; _t677++) {
			const redef = Array_get(redefinitions, _t677)
...
function bait__builder__run_tests(prefs) {
	...
	for (let _t679 = 0; _t679 < prefs.args.length; _t679++) {
		const a = Array_get(prefs.args, _t679)

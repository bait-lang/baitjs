...
function bait__checker__has_toplevel_return(stmts) {
	for (let _t747 = 0; _t747 < stmts.length; _t747++) {
		const stmt = Array_get(stmts, _t747)
...
function bait__checker__Checker_stmts(c, stmts) {
	for (let _t750 = 0; _t750 < stmts.length; _t750++) {
		let stmt = Array_get(stmts, _t750)
...
function bait__checker__Checker_struct_init(c, node) {
	...
	node.name = sym.name
	for (let _t774 = 0; _t774 < node.fields.length; _t774++) {
		const field = Array_get(node.fields, _t774)
...
function bait__checker__Checker_check_init_field_values(c, init, info) {
	for (let _t777 = 0; _t777 < info.fields.length; _t777++) {
		const def_field = Array_get(info.fields, _t777)
		...
		if (!is_required) {
			for (let _t779 = 0; _t779 < def_field.attrs.length; _t779++) {
				const attr = Array_get(def_field.attrs, _t779)
		...
		for (let _t782 = 0; _t782 < init.fields.length; _t782++) {
			const inited = Array_get(init.fields, _t782)
...
function bait__checker__Checker_does_type_exist(c, sym, pos) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.sum_type)) {
		const info = sym.info
		for (let _t807 = 0; _t807 < info.variants.length; _t807++) {
			const variant = Array_get(info.variants, _t807)
	...
	if (eq(sym.kind, bait__ast__TypeKind.fun_)) {
		const info = sym.info
		for (let _t809 = 0; _t809 < info.param_types.length; _t809++) {
			const param_type = Array_get(info.param_types, _t809)
...
function bait__util__escape__linebreak(s) {
	let _t815 = undefined
	if (string_eq(os__platform(), from_js_string("windows"))) {
		_t815 = string_replace(s, from_js_string("\r\n"), from_js_string("\\r\\n"))
	} else {
		_t815 = string_replace(s, from_js_string("\n"), from_js_string("\\n"))
	}
	return _t815
...
function bait__gen__js__Gen_generate_str_fun(g, typ) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.sum_type)) {
		const info = sym.info
		g.fun_decls_out = string_add(g.fun_decls_out, from_js_string(`function ${name.str}(it, indent) {\n`))
		for (let _t825 = 0; _t825 < info.variants.length; _t825++) {
			const bait_var = Array_get(info.variants, _t825)
...
`))
	...
	switch(it) {\n`))
		const info = sym.info
		for (let _t826 = 0; _t826 < info.vals.length; _t826++) {
			const val = Array_get(info.vals, _t826)
...
function bait__gen__js__Gen_get_comptime_val(g, kind, pos) {
	...
	if (eq(kind, bait__token__ComptimeVar.pkg)) {
		...
		_t828 = string_replace(os__abs_path(g.path), from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.file)) {
		_t828 = string_replace(g.path, from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.dir)) {
		_t828 = os__dir(bait__gen__js__Gen_get_comptime_val(g, bait__token__ComptimeVar.abs_file, pos))
	} else if (eq(kind, bait__token__ComptimeVar.line)) {
		_t828 = i32_str(pos.line)
		...
		_t828 = from_js_string(`${file.str}:${line.str}`)
	} else if (eq(kind, bait__token__ComptimeVar.fun_)) {
		_t828 = g.cur_fun.name
	} else if (eq(kind, bait__token__ComptimeVar.baitexe)) {
		_t828 = bait__gen__js__Gen_comptime_baitexe(g)
	} else if (eq(kind, bait__token__ComptimeVar.baitdir)) {
		_t828 = bait__gen__js__Gen_comptime_baitdir(g)
	} else if (eq(kind, bait__token__ComptimeVar.baithash)) {
		_t828 = bait__gen__js__Gen_comptime_baithash(g)
	} else if (eq(kind, bait__token__ComptimeVar.unknown)) {
		_t828 = panic(from_js_string("this should never happen"))
	}
	return _t828
}
...
function bait__gen__js__Gen_fun_decl(g, node) {
	...
	if (i32(node.generic_names.length > 0) && eq(g.cur_concrete_types.length, 0)) {
		...
		for (let _t861 = 0; _t861 < gtypes.length; _t861++) {
			const conc_types = Array_get(gtypes, _t861)
...
function bait__gen__js__gen(files, table, pref) {
	...
	for (let _t887 = 0; _t887 < files.length; _t887++) {
		const file = Array_get(files, _t887)
		g.path = file.path
		g.pkg = file.pkg_name
	...
	for (let _t889 = 0; _t889 < g.table.needed_str_funs.length; _t889++) {
		const typ = Array_get(g.table.needed_str_funs, _t889)
...
function bait__gen__js__Gen_process_imports(g, imports) {
	for (let _t889 = 0; _t889 < imports.length; _t889++) {
		const imp = Array_get(imports, _t889)
...
function bait__gen__js__Gen_headers(g) {
	...
	const _t891 = Map_keys(g.foreign_imports)
	for (let _t892 = 0; _t892 < _t891.length; _t892++) {
		const alias = Array_get(_t891, _t892)
...
function bait__gen__js__Gen_cut_before_any(g, chars) {
	for (let i = i32(g.out.length - 1); i32(i >= 0); i -= 1) {
		for (let _t894 = 0; _t894 < chars.length; _t894++) {
			const c = string_get(chars, _t894)
...
function bait__gen__js__Gen_gen_test_main(g) {
	...
	const _t897 = Map_keys(g.table.fun_decls)
	for (let _t898 = 0; _t898 < _t897.length; _t898++) {
		const key = Array_get(_t897, _t898)
...
function bait__gen__js__Gen_get_concrete_name(g, name, concrete_types) {
	let full_name = name
	for (let _t904 = 0; _t904 < concrete_types.length; _t904++) {
		const t = Array_get(concrete_types, _t904)
...
function bait__gen__js__Gen_stmts(g, stmts) {
	bait__gen__js__Gen_save_stmt_offset(g)
	g.indent += 1
	for (let _t906 = 0; _t906 < stmts.length; _t906++) {
		const stmt = Array_get(stmts, _t906)
...
function bait__gen__js__Gen_enum_decl(g, node) {
	...
	g.indent += 1
	for (let _t919 = 0; _t919 < node.fields.length; _t919++) {
		const field = Array_get(node.fields, _t919)
...
let _t937 = undefined
if (string_eq(os__platform(), from_js_string("windows"))) {
	_t937 = from_js_string("\\r\\n")
} else {
	_t937 = from_js_string("\\n")
}
const bait__gen__c__LB = _t937
...
function bait__gen__c__Gen_generate_str_fun(g, typ) {
	...
	strings__Builder_write(&b, from_c_string("${sym.name.str}{"));\n`))
		...
		for (let _t941 = 0; _t941 < info.fields.length; _t941++) {
			const field = Array_get(info.fields, _t941)
			if (eq(typ, field.typ)) {
...
function bait__gen__c__gen(files, table, pref) {
	...
	for (let _t944 = 0; _t944 < g.table.needed_str_funs.length; _t944++) {
		const typ = Array_get(g.table.needed_str_funs, _t944)
...
function bait__gen__c__Gen_headers(g) {
	...
	for (let _t945 = 0; _t945 < g.foreign_imports.length; _t945++) {
		const name = Array_get(g.foreign_imports, _t945)
...
function bait__gen__c__Gen_get_concrete_name(g, name, concrete_types) {
	let full_name = name
	for (let _t951 = 0; _t951 < concrete_types.length; _t951++) {
		const t = Array_get(concrete_types, _t951)
		full_name = string_add(full_name, string_add(from_js_string("_"), bait__ast__Table_get_sym(g.table, t).name))
	}
...
function bait__gen__c__Gen_get_comptime_val(g, kind, pos) {
	...
	if (eq(kind, bait__token__ComptimeVar.pkg)) {
		...
		_t956 = string_replace(os__abs_path(g.path), from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.file)) {
		_t956 = string_replace(g.path, from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.dir)) {
		_t956 = os__dir(bait__gen__c__Gen_get_comptime_val(g, bait__token__ComptimeVar.abs_file, pos))
	} else if (eq(kind, bait__token__ComptimeVar.line)) {
		_t956 = i32_str(pos.line)
	} else if (eq(kind, bait__token__ComptimeVar.file_line)) {
		...
		_t956 = from_js_string(`${file.str}:${line.str}`)
	} else if (eq(kind, bait__token__ComptimeVar.fun_)) {
		_t956 = g.cur_fun.name
	} else if (eq(kind, bait__token__ComptimeVar.baitexe)) {
		_t956 = bait__gen__c__Gen_comptime_baitexe(g)
	} else if (eq(kind, bait__token__ComptimeVar.baitdir)) {
		_t956 = bait__gen__c__Gen_comptime_baitdir(g)
	} else if (eq(kind, bait__token__ComptimeVar.baithash)) {
		_t956 = bait__gen__c__Gen_comptime_baithash(g)
	} else if (eq(kind, bait__token__ComptimeVar.unknown)) {
		_t956 = panic(from_js_string("this should never happen"))
	}
	return _t956
}
...
function bait__gen__c__Gen_gen_equality_funs(g) {
	for (let _t959 = 0; _t959 < g.table.needed_equality_funs.length; _t959++) {
		const typ = Array_get(g.table.needed_equality_funs, _t959)
...
function bait__gen__c__Gen_array_init(g, node) {
	...
	g.indent += 1
	for (let _t967 = 0; _t967 < node.exprs.length; _t967++) {
		const expr = Array_get(node.exprs, _t967)
		bait__gen__c__Gen_expr(g, expr)
		bait__gen__c__Gen_write(g, from_js_string(", "))
	}
	g.indent -= 1
...
function bait__gen__c__Gen_fun_decl(g, node) {
	...
	if (i32(node.generic_names.length > 0) && eq(g.cur_concrete_types.length, 0)) {
		const gtypes = Map_get_set(g.table.generic_fun_types, bait__ast__FunDecl_key(node), new bait_Array({ data: [], length: 0 }))
		for (let _t986 = 0; _t986 < gtypes.length; _t986++) {
			const conc_types = Array_get(gtypes, _t986)
			for (let i = 0; i < node.generic_names.length; i++) {
				const gn = Array_get(node.generic_names, i)
				Map_set(g.cur_concrete_types, gn, Array_get(conc_types, i))
			}
...
function bait__gen__c__Gen_stmts(g, stmts) {
	bait__gen__c__Gen_save_stmt_offset(g)
	g.indent += 1
	for (let _t1013 = 0; _t1013 < stmts.length; _t1013++) {
		const stmt = Array_get(stmts, _t1013)
...
function bait__gen__c__Gen_typ(g, typ) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.enum_)) {
		_t1025 = from_js_string("enum ")
	} else {
		_t1025 = from_js_string("")
	}
	const prefix = _t1025
...
function bait__gen__c__Gen_write_types(g) {
	for (let _t1025 = 0; _t1025 < g.table.type_symbols.length; _t1025++) {
		const sym = Array_get(g.table.type_symbols, _t1025)
		...
		if (sym.info instanceof bait__ast__StructInfo) {
			...
			for (let _t1027 = 0; _t1027 < info.fields.length; _t1027++) {
				const field = Array_get(info.fields, _t1027)
			...
			for (let _t1029 = 0; _t1029 < info.vals.length; _t1029++) {
				const val = Array_get(info.vals, _t1029)
...
function bait__transformer__Transformer_transform_files(t, files) {
	for (let _t1029 = 0; _t1029 < files.length; _t1029++) {
		let file = Array_get(files, _t1029)
...
function bait__transformer__Transformer_if_match(t, node) {
	for (let _t1034 = 0; _t1034 < node.branches.length; _t1034++) {
		let branch = Array_get(node.branches, _t1034)
...
function bait__transformer__Transformer_call_expr(t, node) {
	bait__transformer__Transformer_expr(t, node.left)
	for (let _t1034 = 0; _t1034 < node.args.length; _t1034++) {
		const arg = Array_get(node.args, _t1034)
...
function bait__builder__Builder_bait_files_in_dir(b, dir) {
	...
	for (let _t1038 = 0; _t1038 < all_files.length; _t1038++) {
		const f = Array_get(all_files, _t1038)
...
function bait__builder__Builder_get_builtin_files(b) {
	let _t1041 = bait__builder__Builder_bait_files_in_dir(b, bait__builder__Builder_resolve_import(b, from_js_string(""), from_js_string("builtin")))
	if (_t1041.is_error) {
		const err = _t1041.msg
		_t1041.data = panic(err)
	}
	return _t1041.data
}
...
function bait__builder__Builder_collect_user_files(b, path) {
	...
	let _t1043 = bait__builder__Builder_bait_files_in_dir(b, path)
	if (_t1043.is_error) {
		return _t1043
	}
	return new Result({ data: _t1043.data })
}
...
function bait__builder__compile(prefs) {
	...
	let _t1044 = bait__builder__Builder_collect_user_files(b, prefs.command)
	if (_t1044.is_error) {
		const err = _t1044.msg
		bait__errors__generic_error(err)
		return 1
	}
	Array_push_many(paths, _t1044.data)
	bait__util__timers__start(from_js_string("PARSE"))
	let ast_files = new bait_Array({ data: [], length: 0 })
	for (let _t1044 = 0; _t1044 < paths.length; _t1044++) {
		const p = Array_get(paths, _t1044)
	...
	for (let i = 0; i32(i < ast_files.length); i += 1) {
		...
		for (let _t1044 = 0; _t1044 < file.imports.length; _t1044++) {
			const imp = Array_get(file.imports, _t1044)
			...
			let _t1048 = bait__builder__Builder_bait_files_in_dir(b, import_dir)
			if (_t1048.is_error) {
				const err = _t1048.msg
				bait__errors__generic_error(from_js_string(`package \`${imp.name.str}\` contains no .bt files`))
				continue
				_t1048.data = new bait_Array({ data: [], length: 0 })
			}
			const imp_paths = _t1048.data
			let nr_newly_parsed = 0
			for (let _t1048 = 0; _t1048 < imp_paths.length; _t1048++) {
				const p = Array_get(imp_paths, _t1048)
	...
	for (let _t1051 = 0; _t1051 < ast_files.length; _t1051++) {
		const f = Array_get(ast_files, _t1051)
		for (let _t1051 = 0; _t1051 < f.imports.length; _t1051++) {
			const imp = Array_get(f.imports, _t1051)
			if (!eq(imp.lang, bait__ast__Language.bait)) {
				continue
			}
	...
	for (let _t1052 = 0; _t1052 < pkg_order.length; _t1052++) {
		const pkg = Array_get(pkg_order, _t1052)
		for (let _t1052 = 0; _t1052 < ast_files.length; _t1052++) {
			const f = Array_get(ast_files, _t1052)
...
function bait__builder__Builder_print_infos(b, infos) {
	...
	for (let _t1065 = 0; _t1065 < infos.length; _t1065++) {
		const info = Array_get(infos, _t1065)
		bait__errors__Message_print(info)
	}
}
...
function bait__builder__Builder_print_errors_and_warnings(b, parser_errs) {
	let nr_warns = 0
	let nr_errors = 0
	for (let _t1065 = 0; _t1065 < b.parsed_files.length; _t1065++) {
		const f = Array_get(b.parsed_files, _t1065)
		nr_warns += f.warnings.length
		nr_errors += f.errors.length
		bait__builder__Builder_print_infos(b, f.infos)
		if (b.prefs.warn_is_error) {
			for (let _t1066 = 0; _t1066 < f.warnings.length; _t1066++) {
				const warn = Array_get(f.warnings, _t1066)
		...
		for (let _t1066 = 0; _t1066 < f.errors.length; _t1066++) {
			const err = Array_get(f.errors, _t1066)
	...
	for (let _t1067 = 0; _t1067 < b.checker.errors.length; _t1067++) {
		const err = Array_get(b.checker.errors, _t1067)
		bait__errors__Message_print(err)
	}
...
function bait__builder__order_pkgs(ordered, pkg, deps, looked) {
	...
	for (let _t1072 = 0; _t1072 < Map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })).length; _t1072++) {
		const d = Array_get(Map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })), _t1072)
...
function bait__builder__run_tests(prefs) {
	...
	for (let _t1075 = 0; _t1075 < prefs.args.length; _t1075++) {
		const a = Array_get(prefs.args, _t1075)
...
const bait__util__FULL_VERSION = from_js_string(`${bait__util__VERSION.str} ${from_js_string("d903ece").str}`)

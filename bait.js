...
function bait__gen__js__Gen_enum_decl(g, node) {
	...
	g.indent += 1
	for (let _t687 = 0; _t687 < node.fields.length; _t687++) {
		const field = Array_get(node.fields, _t687)
...
let _t704 = undefined
if (string_eq(os__platform(), from_js_string("windows"))) {
	_t704 = from_js_string("\\r\\n")
} else {
	_t704 = from_js_string("\\n")
}
const bait__gen__c__LB = _t704
...
function bait__gen__c__Gen_generate_str_fun(g, typ) {
	...
	strings__Builder_write(&b, from_c_string("${sym.name.str}{"));\n`))
		...
		for (let _t708 = 0; _t708 < info.fields.length; _t708++) {
			const field = Array_get(info.fields, _t708)
			if (eq(typ, field.typ)) {
...
function bait__gen__c__Gen_headers(g) {
	...
	for (let _t711 = 0; _t711 < g.foreign_imports.length; _t711++) {
		const name = Array_get(g.foreign_imports, _t711)
...
function bait__gen__c__Gen_cut_before_any(g, chars) {
	for (let i = i32(g.out.length - 1); i32(i >= 0); i -= 1) {
		for (let _t713 = 0; _t713 < chars.length; _t713++) {
			const c = string_get(chars, _t713)
			if (eq(string_get(g.out, i), c)) {
				let cut = string_substr(g.out, i32(i + 1), g.out.length)
				g.out = string_substr(g.out, 0, i)
				return cut
...
function bait__gen__c__Gen_get_comptime_val(g, kind, pos) {
	...
	if (eq(kind, bait__token__ComptimeVar.pkg)) {
		...
		_t716 = string_replace(os__abs_path(g.path), from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.file)) {
		_t716 = string_replace(g.path, from_js_string("\\"), from_js_string("\\\\"))
	} else if (eq(kind, bait__token__ComptimeVar.dir)) {
		_t716 = os__dir(bait__gen__c__Gen_get_comptime_val(g, bait__token__ComptimeVar.abs_file, pos))
	} else if (eq(kind, bait__token__ComptimeVar.line)) {
		_t716 = i32_str(pos.line)
	} else if (eq(kind, bait__token__ComptimeVar.file_line)) {
		...
		_t716 = from_js_string(`${file.str}:${line.str}`)
	} else if (eq(kind, bait__token__ComptimeVar.fun_)) {
		_t716 = g.cur_fun.name
	} else if (eq(kind, bait__token__ComptimeVar.baitexe)) {
		_t716 = bait__gen__c__Gen_comptime_baitexe(g)
	} else if (eq(kind, bait__token__ComptimeVar.baitdir)) {
		_t716 = bait__gen__c__Gen_comptime_baitdir(g)
	} else if (eq(kind, bait__token__ComptimeVar.baithash)) {
		_t716 = bait__gen__c__Gen_comptime_baithash(g)
	} else if (eq(kind, bait__token__ComptimeVar.unknown)) {
		_t716 = panic(from_js_string("this should never happen"))
	}
	return _t716
}
...
function bait__gen__c__Gen_gen_equality_funs(g) {
	for (let _t719 = 0; _t719 < g.table.needed_equality_funs.length; _t719++) {
		const typ = Array_get(g.table.needed_equality_funs, _t719)
		_t720 = bait__gen__c__Gen_equality_fun(g, typ)
	}
}
...
function bait__gen__c__Gen_array_init(g, node) {
	...
	g.indent += 1
	for (let _t728 = 0; _t728 < node.exprs.length; _t728++) {
		const expr = Array_get(node.exprs, _t728)
		bait__gen__c__Gen_expr(g, expr)
		bait__gen__c__Gen_write(g, from_js_string(", "))
	}
	g.indent -= 1
...
function bait__gen__c__Gen_fun_decl(g, node) {
	if (i32(node.generic_names.length > 0) && eq(g.cur_concrete_types.length, 0)) {
		const gtypes = Map_get_set(g.table.generic_fun_types, node.key, new bait_Array({ data: [], length: 0 }))
		for (let _t746 = 0; _t746 < gtypes.length; _t746++) {
			const conc_types = Array_get(gtypes, _t746)
			for (let i = 0; i < node.generic_names.length; i++) {
				const gn = Array_get(node.generic_names, i)
				Map_set(g.cur_concrete_types, gn, Array_get(conc_types, i))
			}
...
function bait__gen__c__Gen_if_match(g, node) {
	...
	for (let i = 0; i < node.branches.length; i++) {
		...
		if (node.is_expr) {
			if (g.empty_line) {
				_t773 = bait__gen__c__Gen_cut_before(g, u8("\n"))
			}
...
function bait__transformer__Transformer_gen_test_main(t) {
	...
	for (let _t792 = 0; _t792 < t.gen_ctx.test_fun_names.length; _t792++) {
		const name = Array_get(t.gen_ctx.test_fun_names, _t792)
...
function bait__transformer__Transformer_transform_files(t, files) {
	for (let _t793 = 0; _t793 < files.length; _t793++) {
		let file = Array_get(files, _t793)
		t.path = file.path
...
function bait__transformer__Transformer_stmt(t, stmt) {
	if (stmt instanceof bait__ast__AssertStmt) {
		...
		_t796 = bait__transformer__Transformer_expr(t, stmt.expr)
	} else if (stmt instanceof bait__ast__EnumDecl) {
	} else if (stmt instanceof bait__ast__ForLoop) {
...
function bait__transformer__Transformer_for_classic_loop(t, node) {
	_t802 = bait__transformer__Transformer_stmt(t, node.init)
	_t803 = bait__transformer__Transformer_stmt(t, node.inc)
	bait__transformer__Transformer_stmts(t, node.stmts)
}
...
function bait__transformer__Transformer_if_match(t, node) {
	for (let _t804 = 0; _t804 < node.branches.length; _t804++) {
		let branch = Array_get(node.branches, _t804)
		bait__transformer__Transformer_stmts(t, branch.stmts)
	}
}

function bait__transformer__Transformer_return_stmt(t, node) {
	_t805 = bait__transformer__Transformer_expr(t, node.expr)
}

function bait__transformer__Transformer_call_expr(t, node) {
	_t806 = bait__transformer__Transformer_expr(t, node.left)
	for (let _t806 = 0; _t806 < node.args.length; _t806++) {
		let arg = Array_get(node.args, _t806)
...
function bait__builder__Builder_bait_files_in_dir(b, dir) {
	...
	for (let _t813 = 0; _t813 < all_files.length; _t813++) {
		const f = Array_get(all_files, _t813)
...
function bait__builder__compile(prefs) {
	...
	for (let _t816 = 0; _t816 < paths.length; _t816++) {
		const p = Array_get(paths, _t816)
	...
	for (let i = 0; i32(i < ast_files.length); i += 1) {
		...
		for (let _t816 = 0; _t816 < file.imports.length; _t816++) {
			const imp = Array_get(file.imports, _t816)
			...
			for (let _t819 = 0; _t819 < imp_paths.length; _t819++) {
				const p = Array_get(imp_paths, _t819)
	...
	if (i32(b.checker.errors.length > 0)) {
		for (let _t823 = 0; _t823 < b.checker.errors.length; _t823++) {
			const err = Array_get(b.checker.errors, _t823)
	...
	for (let _t823 = 0; _t823 < ast_files.length; _t823++) {
		...
		for (let _t823 = 0; _t823 < f.imports.length; _t823++) {
			const imp = Array_get(f.imports, _t823)
			if (!eq(imp.lang, bait__ast__Language.bait)) {
				continue
			}
	...
	for (let _t824 = 0; _t824 < pkg_order.length; _t824++) {
		const pkg = Array_get(pkg_order, _t824)
		for (let _t824 = 0; _t824 < ast_files.length; _t824++) {
			const f = Array_get(ast_files, _t824)
...
function bait__builder__Builder_print_infos(b, infos) {
	...
	for (let _t837 = 0; _t837 < infos.length; _t837++) {
		const info = Array_get(infos, _t837)
...
function bait__builder__Builder_print_errors_and_warnings(b, parser_errs) {
	let nr_warns = 0
	let nr_errors = 0
	for (let _t837 = 0; _t837 < b.parsed_files.length; _t837++) {
		const f = Array_get(b.parsed_files, _t837)
		nr_warns += f.warnings.length
		nr_errors += f.errors.length
		bait__builder__Builder_print_infos(b, f.infos)
		if (b.prefs.warn_is_error) {
			for (let _t838 = 0; _t838 < f.warnings.length; _t838++) {
				const warn = Array_get(f.warnings, _t838)
		...
		for (let _t838 = 0; _t838 < f.errors.length; _t838++) {
			const err = Array_get(f.errors, _t838)
			bait__errors__Message_print(err)
			if (parser_errs) {
				return true
			}
	...
	for (let _t839 = 0; _t839 < b.checker.errors.length; _t839++) {
		const err = Array_get(b.checker.errors, _t839)
		bait__errors__Message_print(err)
	}
...
function bait__builder__run_tests(prefs) {
	...
	for (let _t847 = 0; _t847 < prefs.args.length; _t847++) {
		const a = Array_get(prefs.args, _t847)

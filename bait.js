...
function bait__lexer__Lexer({ val = from_js_string(""), comment_mode = 0, text = from_js_string(""), insert_semi = false, line_starts = new bait_Array({ data: [], length: 0 }), start = 0, pos = 0, is_str_interpol = false, is_str_dollar = false, str_quote = 0 }) {
	this.val = val
...
function bait__lexer__Lexer_simple_token(l, c) {
	...
	if (eq(c, u8("\n"))) {
		...
		if (eq(c2, u8("/"))) {
			return bait__lexer__Lexer_comment(l)
		}
...
function bait__lexer__Lexer_comment(l) {
	const start = i32(l.pos + 1)
	bait__lexer__Lexer_skip_line(l)
	if (eq(l.comment_mode, bait__lexer__CommentMode.store)) {
...
function bait__lexer__Lexer_unexpected_char(l, c) {
	l.val = from_js_string(`unexpected char ${u8_ascii(c).str}`)
	return bait__token__Token.error
}
...
function bait__parser__Parser_for_in_loop(p, label, pos) {
	...
	let valvar = new bait__ast__Param({ pos: p.pos, name: bait__parser__Parser_check_name(p), is_mut: is_mut })
...
function bait__parser__Parser_error(p, msg) {
	Array_push(p.errors, new bait__errors__Message({ kind: bait__errors__Kind.error, path: p.path, pos: p.pos, title: from_js_string("error"), msg: msg }))
	p.should_abort = true
}
...
const bait__util__FULL_VERSION = from_js_string(`${bait__util__VERSION.str} ${from_js_string("d195b90").str}`)
...
function bait__builder__get_project_root(abs_dir) {
	...
	if (Array_contains(files, from_js_string("bait.toml"))) {
		return abs_dir
	}
...
function bait__builder__order_pkgs(ordered, pkg, deps, looked) {
	...
	for (let _t810 = 0; _t810 < map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })).length; _t810++) {
		const d = Array_get(map_get_set(deps, pkg, new bait_Array({ data: [], length: 0 })), _t810)
...
function bait__builder__run_tests(prefs) {
	...
	for (let _t814 = 0; _t814 < prefs.args.length; _t814++) {
		const a = Array_get(prefs.args, _t814)

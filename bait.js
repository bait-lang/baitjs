...
function bait__parser__Parser_expr(p, precedence) {
	let _t139 = bait__parser__Parser_single_expr(p)
	if (_t139.is_error) {
		return _t139
	}
	let node = _t139.data
	while (precedence < bait__token__Token_precedence(p.tok)) {
		if (eq(p.tok, bait__token__Token.dot)) {
			let _t141 = bait__parser__Parser_dot_expr(p, node)
			if (_t141.is_error) {
				return _t141
			}
			node = _t141.data
		} else if (eq(p.tok, bait__token__Token.lsqr)) {
			let _t142 = bait__parser__Parser_index_expr(p, node)
			if (_t142.is_error) {
				return _t142
			}
			node = _t142.data
		} else if (eq(p.tok, bait__token__Token.key_as)) {
			let _t143 = bait__parser__Parser_as_cast(p, node)
			if (_t143.is_error) {
				return _t143
			}
			node = _t143.data
		} else if (bait__token__Token_is_infix(p.tok)) {
			let _t144 = bait__parser__Parser_infix_expr(p, node)
			if (_t144.is_error) {
				return _t144
			}
			node = _t144.data
...
function bait__parser__Parser_single_expr(p) {
	if (eq(p.tok, bait__token__Token.hash)) {
		let _t146 = bait__parser__Parser_hash_expr(p)
		if (_t146.is_error) {
			return _t146
		}
		...
		let _t147 = bait__parser__Parser_comptime_expr(p)
		if (_t147.is_error) {
			return _t147
		}
		return new Result({ data: _t147.data })
	} else if (eq(p.tok, bait__token__Token.dot)) {
		let _t148 = bait__parser__Parser_enum_val(p, false)
		if (_t148.is_error) {
			return _t148
		}
		...
		let _t154 = bait__parser__Parser_if_match(p, true)
		if (_t154.is_error) {
			return _t154
		}
		return new Result({ data: _t154.data })
	} else if (eq(p.tok, bait__token__Token.key_match)) {
		let _t155 = bait__parser__Parser_match_as_if_expr(p, true)
		if (_t155.is_error) {
			return _t155
		}
		return new Result({ data: _t155.data })
	} else if (eq(p.tok, bait__token__Token.key_mut)) {
		let _t156 = bait__parser__Parser_ident(p, bait__ast__Language.bait)
		if (_t156.is_error) {
			return _t156
		}
...
_t159.data
		...
		let _t161 = bait__parser__Parser_error(p, msg)
		if (_t161.is_error) {
			return _t161
		}
_t161.data
...
function bait__parser__Parser_array_init(p) {
	const pos = p.pos
	let _t164 = bait__parser__Parser_check(p, bait__token__Token.lsqr)
	if (_t164.is_error) {
		return _t164
	}
_t164.data
	if (eq(p.tok, bait__token__Token.rsqr)) {
		bait__parser__Parser_next(p)
		let _t166 = bait__parser__Parser_parse_type(p)
		if (_t166.is_error) {
			return _t166
		}
		const elem_type = _t166.data
		...
		if (eq(p.tok, bait__token__Token.lcur)) {
			...
			while (!eq(p.tok, bait__token__Token.rcur)) {
				let _t168 = bait__parser__Parser_check_name(p)
				if (_t168.is_error) {
					return _t168
				}
				const key = _t168.data
				let _t169 = bait__parser__Parser_check(p, bait__token__Token.assign)
				if (_t169.is_error) {
					return _t169
				}
_t169.data
				...
				const expr = _t170.data
				if (string_eq(key, from_js_string("length"))) {
					length_expr = expr
				} else if (string_eq(key, from_js_string("cap"))) {
					cap_expr = expr
				} else {
					let _t172 = bait__parser__Parser_error(p, from_js_string(`invalid array init field: ${key.str}`))
					if (_t172.is_error) {
						return _t172
					}
_t172.data
			...
			let _t173 = bait__parser__Parser_check(p, bait__token__Token.rcur)
			if (_t173.is_error) {
				return _t173
			}
_t173.data
	...
	while (!eq(p.tok, bait__token__Token.rsqr)) {
		let _t174 = bait__parser__Parser_expr(p, bait__token__Precedence.lowest)
		if (_t174.is_error) {
			return _t174
		}
		Array_push(exprs, _t174.data)
		if (eq(p.tok, bait__token__Token.comma)) {
			bait__parser__Parser_next(p)
		}
	}
	let _t176 = bait__parser__Parser_check(p, bait__token__Token.rsqr)
	if (_t176.is_error) {
		return _t176
	}
_t176.data
...
function bait__parser__Parser_as_cast(p, left) {
	const pos = p.pos
	let _t177 = bait__parser__Parser_check(p, bait__token__Token.key_as)
	if (_t177.is_error) {
		return _t177
	}
_t177.data
	let _t178 = bait__parser__Parser_parse_type(p)
	if (_t178.is_error) {
		return _t178
	}
	const bait_target = _t178.data
...
function bait__parser__Parser_comptime_expr(p) {
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.key_if)) {
		let _t180 = bait__parser__Parser_if_match(p, false)
		if (_t180.is_error) {
			return _t180
		}
		return new Result({ data: _t180.data })
	}
	let _t181 = bait__parser__Parser_comptime_var(p)
	if (_t181.is_error) {
		return _t181
	}
...
function bait__parser__Parser_hash_expr(p) {
	...
	if (!eq(p.tok, bait__token__Token.string)) {
		...
		let _t195 = bait__parser__Parser_name_expr(p, lang)
		if (_t195.is_error) {
			return _t195
		}
		return new Result({ data: _t195.data })
	}
	let _t196 = bait__parser__Parser_string_literal(p, bait__ast__Language.bait)
	if (_t196.is_error) {
		return _t196
	}
	const str_node = _t196.data
...
function bait__parser__Parser_ident(p, lang) {
	...
	const name = bait__parser__Parser_prepend_expr_pkg(p, _t199.data)
...
function bait__parser__Parser_index_expr(p, left) {
	const pos = p.pos
	let _t200 = bait__parser__Parser_check(p, bait__token__Token.lsqr)
	if (_t200.is_error) {
		return _t200
	}
_t200.data
...
function bait__parser__Parser_infix_expr(p, left) {
	...
	const op_tok = p.tok
	bait__parser__Parser_next(p)
	let _t203 = bait__parser__Parser_expr(p, bait__token__Token_precedence(op_tok))
	if (_t203.is_error) {
		return _t203
	}
	const right = _t203.data
...
function bait__parser__Parser_map_init(p) {
	...
	if (eq(p.tok, bait__token__Token.lsqr)) {
		let _t205 = bait__parser__Parser_parse_map_type(p)
		if (_t205.is_error) {
			return _t205
		}
		const map_type = _t205.data
	...
	let _t206 = bait__parser__Parser_check(p, bait__token__Token.lcur)
	if (_t206.is_error) {
		return _t206
	}
_t206.data
	while (!eq(p.tok, bait__token__Token.rcur)) {
		...
		Array_push(keys, _t207.data)
		let _t208 = bait__parser__Parser_check(p, bait__token__Token.colon)
		if (_t208.is_error) {
			return _t208
		}
_t208.data
...
function bait__parser__Parser_name_expr(p, lang) {
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.dot)) {
		if (eq(lang, bait__ast__Language.js)) {
			...
			p.expr_pkg = string_add(p.expr_pkg, string_add(from_js_string("."), _t212.data))
			let _t213 = bait__parser__Parser_check(p, bait__token__Token.dot)
			if (_t213.is_error) {
				return _t213
			}
_t213.data
	...
	if (string_eq(p.val, from_js_string("map"))) {
		let _t215 = bait__parser__Parser_map_init(p)
		if (_t215.is_error) {
			return _t215
		}
	...
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.lpar)) {
		let _t217 = bait__parser__Parser_fun_call(p, lang)
		if (_t217.is_error) {
			return _t217
		}
	...
	if (p.is_struct_possible && (eq(bait__parser__Parser_peek(p), bait__token__Token.lcur) || eq(bait__parser__Parser_peek(p), bait__token__Token.lsqr))) {
		if (capitalised || Array_contains_string(new bait_Array({ data: [from_js_string("array"), from_js_string("map"), from_js_string("string")], length: 3 }), p.val)) {
			let _t220 = bait__parser__Parser_struct_init(p)
			if (_t220.is_error) {
				return _t220
			}
	...
	if (capitalised && eq(bait__parser__Parser_peek(p), bait__token__Token.dot)) {
		let _t222 = bait__parser__Parser_enum_val(p, true)
		if (_t222.is_error) {
			return _t222
		}
	...
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.string) && string_eq(p.val, from_js_string("js"))) {
		bait__parser__Parser_next(p)
		let _t224 = bait__parser__Parser_string_literal(p, bait__ast__Language.js)
		if (_t224.is_error) {
			return _t224
		}
		return new Result({ data: _t224.data })
	}
	let _t225 = bait__parser__Parser_ident(p, lang)
	if (_t225.is_error) {
		return _t225
	}
...
function bait__parser__Parser_par_expr(p) {
	...
	const expr = _t226.data
	let _t227 = bait__parser__Parser_check(p, bait__token__Token.rpar)
	if (_t227.is_error) {
		return _t227
	}
_t227.data
...
function bait__parser__Parser_prefix_expr(p) {
	...
	const op = p.tok
	bait__parser__Parser_next(p)
	let _t228 = bait__parser__Parser_expr(p, bait__token__Precedence.prefix)
	if (_t228.is_error) {
		return _t228
	}
	const right = _t228.data
...
function bait__parser__Parser_string_literal(p, lang) {
	...
	while (eq(p.tok, bait__token__Token.string)) {
		...
		bait__parser__Parser_next(p)
		let _t231 = bait__parser__Parser_check(p, bait__token__Token.lcur)
		if (_t231.is_error) {
			return _t231
		}
_t231.data
...
function bait__parser__Parser_typeof_expr(p) {
	const pos = p.pos
	let _t233 = bait__parser__Parser_check(p, bait__token__Token.key_typeof)
	if (_t233.is_error) {
		return _t233
	}
_t233.data
	let _t234 = bait__parser__Parser_check(p, bait__token__Token.lpar)
	if (_t234.is_error) {
		return _t234
	}
_t234.data
...
function bait__parser__Parser_for_loop(p, label) {
	...
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.decl_assign)) {
		let _t238 = bait__parser__Parser_for_classic_loop(p, label, pos)
		if (_t238.is_error) {
			return _t238
		}
	...
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.key_in) || eq(bait__parser__Parser_peek(p), bait__token__Token.comma) || eq(p.tok, bait__token__Token.key_mut)) {
		let _t240 = bait__parser__Parser_for_in_loop(p, label, pos)
		if (_t240.is_error) {
			return _t240
		}
...
function bait__parser__Parser_for_classic_loop(p, label, pos) {
	p.is_for_init = true
	let _t243 = bait__parser__Parser_assign_stmt(p)
	if (_t243.is_error) {
		return _t243
	}
	const init = _t243.data
	p.is_for_init = false
	let _t244 = bait__parser__Parser_check(p, bait__token__Token.semicolon)
	if (_t244.is_error) {
		return _t244
	}
_t244.data
	let _t245 = bait__parser__Parser_expr(p, bait__token__Precedence.lowest)
	if (_t245.is_error) {
		return _t245
	}
	const cond = _t245.data
	let _t246 = bait__parser__Parser_check(p, bait__token__Token.semicolon)
	if (_t246.is_error) {
		return _t246
	}
_t246.data
	let _t247 = bait__parser__Parser_stmt(p)
	if (_t247.is_error) {
		return _t247
	}
	const inc = _t247.data
	let _t248 = bait__parser__Parser_parse_block(p)
	if (_t248.is_error) {
		return _t248
	}
	const stmts = _t248.data
...
function bait__parser__Parser_for_in_loop(p, label, pos) {
	...
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.comma)) {
		let _t250 = bait__parser__Parser_check_name(p)
		if (_t250.is_error) {
			return _t250
		}
		idxvar = _t250.data
	...
	let valvar = new bait__ast__Param({ pos: p.pos, name: _t252.data, is_mut: is_mut })
	let _t253 = bait__parser__Parser_check(p, bait__token__Token.key_in)
	if (_t253.is_error) {
		return _t253
	}
_t253.data
...
function bait__parser__Parser_fun_decl(p) {
	...
	let _t256 = bait__parser__Parser_check(p, bait__token__Token.key_fun)
	if (_t256.is_error) {
		return _t256
	}
_t256.data
	...
	if (eq(p.tok, bait__token__Token.lpar)) {
		...
		const rec_name = _t259.data
		let _t260 = bait__parser__Parser_parse_lang(p)
		if (_t260.is_error) {
			return _t260
		}
		lang = _t260.data
		...
		if (eq(lang, bait__ast__Language.bait)) {
					let _t262 = bait__parser__Parser_parse_type(p)
			if (_t262.is_error) {
				return _t262
			}
			_t261 = _t262.data
		} else {
					let _t263 = bait__parser__Parser_parse_name_type(p, lang)
			if (_t263.is_error) {
				return _t263
			}
			_t261 = _t263.data
		...
		let _t265 = bait__parser__Parser_check(p, bait__token__Token.rpar)
		if (_t265.is_error) {
			return _t265
		}
_t265.data
	...
	if (eq(p.tok, bait__token__Token.hash)) {
		...
		if (eq(lang, bait__ast__Language.js) && eq(bait__parser__Parser_peek(p), bait__token__Token.dot)) {
			let _t269 = bait__parser__Parser_check_name(p)
			if (_t269.is_error) {
				return _t269
			}
			name = string_add(name, string_add(_t269.data, from_js_string(".")))
			let _t270 = bait__parser__Parser_check(p, bait__token__Token.dot)
			if (_t270.is_error) {
				return _t270
			}
_t270.data
	...
	name = string_add(name, _t271.data)
	const is_test = string_starts_with(name, from_js_string("test_"))
	if (!is_method && eq(lang, bait__ast__Language.bait)) {
		name = bait__parser__Parser_prepend_pkg(p, name)
	}
	let _t273 = bait__parser__Parser_generic_type_names(p)
	if (_t273.is_error) {
		return _t273
	}
	const generic_names = _t273.data
	let _t274 = bait__parser__Parser_check(p, bait__token__Token.lpar)
	if (_t274.is_error) {
		return _t274
	}
_t274.data
	let _t275 = bait__parser__Parser_fun_params(p)
	if (_t275.is_error) {
		return _t275
	}
	Array_push_many(params, _t275.data)
...
_t276.data
	...
	if (!eq(p.tok, bait__token__Token.lcur) && eq(pos.line, p.pos.line)) {
		let _t278 = bait__parser__Parser_parse_type(p)
		if (_t278.is_error) {
			return _t278
		}
		return_type = _t278.data
	}
	...
	if (is_method) {
		...
		if (eq(lang, bait__ast__Language.bait) && bait__ast__TypeSymbol_has_method(sym, name)) {
			let _t281 = bait__parser__Parser_error(p, from_js_string(`Method "${name.str}" already exists on type "${sym.name.str}"`))
			if (_t281.is_error) {
				return _t281
			}
_t281.data
		...
		for (let _t282 = 0; _t282 < params.length; _t282++) {
			const param = Array_get(params, _t282)
...
function bait__parser__Parser_anon_fun(p) {
	const pos = p.pos
	let _t285 = bait__parser__Parser_check(p, bait__token__Token.key_fun)
	if (_t285.is_error) {
		return _t285
	}
_t285.data
	let _t286 = bait__parser__Parser_check(p, bait__token__Token.lpar)
	if (_t286.is_error) {
		return _t286
	}
_t286.data
	let _t287 = bait__parser__Parser_fun_params(p)
	if (_t287.is_error) {
		return _t287
	}
	const params = _t287.data
	let _t288 = bait__parser__Parser_check(p, bait__token__Token.rpar)
	if (_t288.is_error) {
		return _t288
	}
_t288.data
	let return_type = bait__ast__VOID_TYPE
	if (!eq(p.tok, bait__token__Token.lcur)) {
		let _t290 = bait__parser__Parser_parse_type(p)
		if (_t290.is_error) {
			return _t290
		}
		return_type = _t290.data
	}
	let param_types = new bait_Array({ data: [], length: 0 })
...
function bait__parser__Parser_fun_params(p) {
	...
	while (!eq(p.tok, bait__token__Token.rpar)) {
		...
		let _t294 = bait__parser__Parser_check_name(p)
		if (_t294.is_error) {
			return _t294
		}
		...
		const param = new bait__ast__Param({ is_mut: is_mut, name: _t294.data, typ: _t295.data, pos: pos })
		Array_push(params, param)
		if (!eq(p.tok, bait__token__Token.rpar)) {
			let _t297 = bait__parser__Parser_check(p, bait__token__Token.comma)
			if (_t297.is_error) {
				return _t297
			}
_t297.data
...
function bait__parser__Parser_fun_call(p, lang) {
	const pos = p.pos
	let _t298 = bait__parser__Parser_check_name(p)
	if (_t298.is_error) {
		return _t298
	}
	const name = bait__parser__Parser_prepend_expr_pkg(p, _t298.data)
	let _t299 = bait__parser__Parser_check(p, bait__token__Token.lpar)
	if (_t299.is_error) {
		return _t299
	}
_t299.data
	let _t300 = bait__parser__Parser_call_args(p)
	if (_t300.is_error) {
		return _t300
	}
	const args = _t300.data
	let _t301 = bait__parser__Parser_check(p, bait__token__Token.rpar)
	if (_t301.is_error) {
		return _t301
	}
_t301.data
	let _t302 = bait__parser__Parser_or_block(p)
	if (_t302.is_error) {
		return _t302
	}
	const or_block = _t302.data
...
function bait__parser__Parser_call_args(p) {
	...
	while (!eq(p.tok, bait__token__Token.rpar)) {
		...
		let _t308 = bait__parser__Parser_expr(p, bait__token__Precedence.lowest)
		if (_t308.is_error) {
			return _t308
		}
		Array_push(args, new bait__ast__CallArg({ is_mut: is_mut, expr: _t308.data }))
		if (!eq(p.tok, bait__token__Token.rpar)) {
			if (eq(p.tok, bait__token__Token.eof)) {
				break
			}
			let _t311 = bait__parser__Parser_check(p, bait__token__Token.comma)
			if (_t311.is_error) {
				return _t311
			}
_t311.data
...
function bait__parser__Parser_or_block(p) {
	...
	bait__parser__Parser_next(p)
	let _t314 = bait__parser__Parser_parse_block(p)
	if (_t314.is_error) {
		return _t314
	}
	const stmts = _t314.data
...
function bait__parser__Parser_if_match(p, is_expr) {
	...
	while (true) {
		const bpos = p.pos
		if (is_comptime) {
			let _t316 = bait__parser__Parser_check(p, bait__token__Token.dollar)
			if (_t316.is_error) {
				return _t316
			}
_t316.data
...
_t321.data
		...
		let _t322 = bait__parser__Parser_check(p, bait__token__Token.key_if)
		if (_t322.is_error) {
			return _t322
		}
_t322.data
		p.is_struct_possible = false
		let _t323 = bait__parser__Parser_expr(p, bait__token__Precedence.lowest)
		if (_t323.is_error) {
			return _t323
		}
		const cond = _t323.data
		p.is_struct_possible = true
...
function bait__parser__Parser_match_as_if_expr(p, is_expr) {
	...
	let has_else = false
	let branches = new bait_Array({ data: [], length: 0 })
	let _t326 = bait__parser__Parser_check(p, bait__token__Token.key_match)
	if (_t326.is_error) {
		return _t326
	}
_t326.data
	p.is_struct_possible = false
	let _t327 = bait__parser__Parser_expr(p, bait__token__Precedence.lowest)
	if (_t327.is_error) {
		return _t327
	}
	const cond_left = _t327.data
	p.is_struct_possible = true
	let _t328 = bait__parser__Parser_check(p, bait__token__Token.lcur)
	if (_t328.is_error) {
		return _t328
	}
_t328.data
	while (!eq(p.tok, bait__token__Token.rcur)) {
		...
		p.is_struct_possible = false
		let _t331 = bait__parser__Parser_expr_list(p)
		if (_t331.is_error) {
			return _t331
		}
		const right_conds = _t331.data
		p.is_struct_possible = true
	...
	let _t333 = bait__parser__Parser_check(p, bait__token__Token.rcur)
	if (_t333.is_error) {
		return _t333
	}
_t333.data
...
function bait__parser__Parser_foreign_import(p, pos) {
	const name = p.val
	bait__parser__Parser_next(p)
	let _t336 = bait__parser__Parser_check(p, bait__token__Token.key_as)
	if (_t336.is_error) {
		return _t336
	}
_t336.data
	let _t337 = bait__parser__Parser_parse_lang(p)
	if (_t337.is_error) {
		return _t337
	}
	const lang = _t337.data
	...
	if (eq(lang, bait__ast__Language.js)) {
		...
		alias = bait__ast__Language_prepend_to(lang, _t339.data)
	} else if (eq(lang, bait__ast__Language.c)) {
		let _t340 = bait__parser__Parser_check_name(p)
		if (_t340.is_error) {
			return _t340
		}
_t340.data
...
function bait__parser__Parser_bait_import(p, pos) {
	...
	let name_parts = new bait_Array({ data: [_t341.data], length: 1 })
	while (eq(p.tok, bait__token__Token.dot)) {
		...
		Array_push(name_parts, _t342.data)
	}
...
function bait__parser__Parser_import_stmts(p) {
	...
	while (eq(p.tok, bait__token__Token.key_import)) {
		...
		let _t345 = undefined
		if (eq(p.tok, bait__token__Token.string)) {
					let _t346 = bait__parser__Parser_foreign_import(p, pos)
			if (_t346.is_error) {
				return _t346
			}
			_t345 = _t346.data
		} else {
					let _t347 = bait__parser__Parser_bait_import(p, pos)
			if (_t347.is_error) {
				return _t347
			}
			_t345 = _t347.data
...
function bait__parser__Parser({ pref = new bait__preference__Prefs({}), path = from_js_string(""), eofs = 0, tok = 0, next_tok = 0, val = from_js_string(""), pos = new bait__token__Pos({}), pkg_name = from_js_string(""), import_aliases = new map({ data: new Map([]), length: 0 }), attributes = new bait_Array({ data: [], length: 0 }), expr_pkg = from_js_string(""), is_for_init = false, is_struct_possible = false, table = new bait__ast__Table({}), lexer = new bait__lexer__Lexer({}), infos = new bait_Array({ data: [], length: 0 }), warnings = new bait_Array({ data: [], length: 0 }), error = new bait__errors__Message({}) }) {
	this.pref = pref
	this.path = path
	this.eofs = eofs
	this.tok = tok
...
function bait__parser__Parser_parse(p) {
	let _t348 = bait__parser__Parser_parse_result(p)
	if (_t348.is_error) {
		 const err = _t348.msg
		_t348.data = bait__parser__Parser_file_error(p)
	}
	return _t348.data
}

function bait__parser__Parser_parse_result(p) {
	let _t349 = bait__parser__Parser_parse_attributes(p)
	if (_t349.is_error) {
		return _t349
	}
_t349.data
	let _t350 = bait__parser__Parser_package_decl(p)
	if (_t350.is_error) {
		return _t350
	}
	p.pkg_name = _t350.data
	...
	let _t353 = bait__parser__Parser_import_stmts(p)
	if (_t353.is_error) {
		return _t353
	}
	const imports = _t353.data
...
_t354.data
		let _t355 = bait__parser__Parser_toplevel_stmt(p)
		if (_t355.is_error) {
			return _t355
		}
		Array_push(stmts, _t355.data)
	}
...
function bait__parser__Parser_parse_attributes(p) {
	while (eq(p.tok, bait__token__Token.attr)) {
		...
		if (eq(p.tok, bait__token__Token.lpar)) {
			bait__parser__Parser_next(p)
			value = p.val
			let _t362 = bait__parser__Parser_check(p, bait__token__Token.string)
			if (_t362.is_error) {
				return _t362
			}
_t362.data
			let _t363 = bait__parser__Parser_check(p, bait__token__Token.rpar)
			if (_t363.is_error) {
				return _t363
			}
_t363.data
...
function bait__parser__Parser_parse_block(p) {
	let _t364 = bait__parser__Parser_check(p, bait__token__Token.lcur)
	if (_t364.is_error) {
		return _t364
	}
_t364.data
	...
	while (!eq(p.tok, bait__token__Token.rcur) && !eq(p.tok, bait__token__Token.eof)) {
		let _t365 = bait__parser__Parser_stmt(p)
		if (_t365.is_error) {
			return _t365
		}
		Array_push(stmts, _t365.data)
	}
	let _t366 = bait__parser__Parser_check(p, bait__token__Token.rcur)
	if (_t366.is_error) {
		return _t366
	}
_t366.data
...
function bait__parser__Parser_check(p, expected) {
	if (!eq(p.tok, expected)) {
		let _t369 = bait__parser__Parser_error(p, from_js_string(`unexpected ${bait__token__Token_str(p.tok).str}, expecting ${bait__token__Token_str(expected).str}`))
		if (_t369.is_error) {
			return _t369
		}
_t369.data
...
function bait__parser__Parser_check_name(p) {
	const val = p.val
	let _t370 = bait__parser__Parser_check(p, bait__token__Token.name)
	if (_t370.is_error) {
		return _t370
	}
_t370.data
...
function bait__parser__Parser_parse_lang(p) {
	...
	bait__parser__Parser_next(p)
	let _t373 = bait__parser__Parser_check(p, bait__token__Token.dot)
	if (_t373.is_error) {
		return _t373
	}
_t373.data
...
function bait__parser__Parser_toplevel_stmt(p) {
	let _t378 = undefined
	if (eq(p.tok, bait__token__Token.key_pub)) {
			let _t379 = bait__parser__Parser_pub_stmt(p)
		if (_t379.is_error) {
			return _t379
		}
		_t378 = _t379.data
	} else if (eq(p.tok, bait__token__Token.hash)) {
			let _t380 = bait__parser__Parser_expr_stmt(p)
		if (_t380.is_error) {
			return _t380
		}
		_t378 = _t380.data
	} else if (eq(p.tok, bait__token__Token.key_const)) {
			let _t381 = bait__parser__Parser_const_decl(p)
		if (_t381.is_error) {
			return _t381
		}
		_t378 = _t381.data
	} else if (eq(p.tok, bait__token__Token.key_enum)) {
			let _t382 = bait__parser__Parser_enum_decl(p)
		if (_t382.is_error) {
			return _t382
		}
		_t378 = _t382.data
	} else if (eq(p.tok, bait__token__Token.key_fun)) {
			let _t383 = bait__parser__Parser_fun_decl(p)
		if (_t383.is_error) {
			return _t383
		}
		_t378 = _t383.data
	} else if (eq(p.tok, bait__token__Token.key_global)) {
			let _t384 = bait__parser__Parser_global_decl(p)
		if (_t384.is_error) {
			return _t384
		}
		_t378 = _t384.data
		...
		_t378 = _t386.data
			...
			let _t387 = bait__parser__Parser_type_decl(p)
		if (_t387.is_error) {
			return _t387
		}
		_t378 = _t387.data
	} else {
			let _t388 = bait__parser__Parser_script_mode_or_error(p)
		if (_t388.is_error) {
			return _t388
		}
		_t378 = _t388.data
	}
	return new Result({ data: _t378 })
}
...
function bait__parser__Parser_pub_stmt(p) {
	...
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.key_const)) {
		...
		_t389 = _t394.data
			...
			let _t395 = bait__parser__Parser_error(p, from_js_string(`cannot use pub keyword before ${bait__token__Token_str(bait__parser__Parser_peek(p)).str}`))
		if (_t395.is_error) {
			return _t395
		}
_t395.data
		_t389 = new bait__ast__EmptyStmt({})
	}
...
function bait__parser__Parser_script_mode_or_error(p) {
	if (string_eq(p.pkg_name, from_js_string("main"))) {
		let _t397 = bait__parser__Parser_script_mode_main(p)
		if (_t397.is_error) {
			return _t397
		}
		return new Result({ data: _t397.data })
	}
	let _t398 = bait__parser__Parser_error(p, from_js_string(`bad toplevel token: kind = ${bait__token__Token_str(p.tok).str}, val = ${p.val.str}`))
	if (_t398.is_error) {
		return _t398
	}
_t398.data
...
function bait__parser__Parser_script_mode_main(p) {
	...
	let stmts = new bait_Array({ data: [], length: 0 })
	while (!eq(p.tok, bait__token__Token.eof)) {
		let _t400 = bait__parser__Parser_stmt(p)
		if (_t400.is_error) {
			return _t400
		}
		Array_push(stmts, _t400.data)
	}
...
function bait__parser__Parser_stmt(p) {
	...
	if (eq(p.tok, bait__token__Token.name)) {
			let _t402 = bait__parser__Parser_stmt_with_name(p)
		if (_t402.is_error) {
			return _t402
		}
		_t401 = _t402.data
	} else if (eq(p.tok, bait__token__Token.key_assert)) {
			let _t403 = bait__parser__Parser_assert_stmt(p)
		if (_t403.is_error) {
			return _t403
		}
		_t401 = _t403.data
	} else if (eq(p.tok, bait__token__Token.key_break) || eq(p.tok, bait__token__Token.key_continue)) {
			let _t404 = bait__parser__Parser_loop_control_stmt(p)
		if (_t404.is_error) {
			return _t404
		}
		_t401 = _t404.data
	} else if (eq(p.tok, bait__token__Token.key_for)) {
			let _t405 = bait__parser__Parser_for_loop(p, from_js_string(""))
		if (_t405.is_error) {
			return _t405
		}
		_t401 = _t405.data
	} else if (eq(p.tok, bait__token__Token.key_if)) {
			let _t406 = bait__parser__Parser_if_match(p, false)
		if (_t406.is_error) {
			return _t406
		}
		_t401 = _t406.data
	} else if (eq(p.tok, bait__token__Token.key_match)) {
			let _t407 = bait__parser__Parser_match_as_if_expr(p, false)
		if (_t407.is_error) {
			return _t407
		}
		_t401 = _t407.data
	} else if (eq(p.tok, bait__token__Token.key_mut)) {
			let _t408 = bait__parser__Parser_assign_stmt(p)
		if (_t408.is_error) {
			return _t408
		}
		_t401 = _t408.data
	} else if (eq(p.tok, bait__token__Token.key_return)) {
			let _t409 = bait__parser__Parser_return_stmt(p)
		if (_t409.is_error) {
			return _t409
		}
		_t401 = _t409.data
...
function bait__parser__Parser_stmt_with_name(p) {
	if (eq(bait__parser__Parser_peek(p), bait__token__Token.colon)) {
		...
		bait__parser__Parser_next(p)
		bait__parser__Parser_next(p)
		let _t412 = bait__parser__Parser_for_loop(p, label)
		if (_t412.is_error) {
			return _t412
		}
		return new Result({ data: _t412.data })
	}
	let _t413 = bait__parser__Parser_expr(p, bait__token__Precedence.lowest)
	if (_t413.is_error) {
		return _t413
	}
	const left = _t413.data
	if (bait__token__Token_is_assign(p.tok)) {
		let _t415 = bait__parser__Parser_partial_assign_stmt(p, left)
		if (_t415.is_error) {
			return _t415
		}
		return new Result({ data: _t415.data })
	}
...
function bait__parser__Parser_assert_stmt(p) {
	bait__parser__Parser_next(p)
	const pos = p.pos
	let _t416 = bait__parser__Parser_expr(p, bait__token__Precedence.lowest)
	if (_t416.is_error) {
		return _t416
	}
	const expr = _t416.data
...
function bait__parser__Parser_assign_stmt(p) {
	...
	const left = _t417.data
	let _t418 = bait__parser__Parser_partial_assign_stmt(p, left)
	if (_t418.is_error) {
		return _t418
	}
	return new Result({ data: _t418.data })
}
...
function bait__parser__Parser_const_decl(p) {
	...
	const is_pub = bait__parser__Parser_check_pub(p)
	bait__parser__Parser_next(p)
	let _t421 = bait__parser__Parser_parse_lang(p)
	if (_t421.is_error) {
		return _t421
	}
	const lang = _t421.data
	let name = from_js_string("")
	if (eq(lang, bait__ast__Language.js) && eq(bait__parser__Parser_peek(p), bait__token__Token.dot)) {
		...
		name = string_add(_t423.data, from_js_string("."))
		bait__parser__Parser_next(p)
	}
	...
	name = string_add(name, _t424.data)
	if (eq(lang, bait__ast__Language.bait)) {
		name = bait__parser__Parser_prepend_pkg(p, name)
	}
	let _t426 = bait__parser__Parser_check(p, bait__token__Token.decl_assign)
	if (_t426.is_error) {
		return _t426
	}
_t426.data
	...
	if (eq(lang, bait__ast__Language.bait)) {
		...
		let _t429 = bait__parser__Parser_parse_type(p)
		if (_t429.is_error) {
			return _t429
		}
		typ = _t429.data
...
function bait__parser__Parser_enum_decl(p) {
	...
	const is_pub = bait__parser__Parser_check_pub(p)
	bait__parser__Parser_next(p)
	let _t433 = bait__parser__Parser_parse_lang(p)
	if (_t433.is_error) {
		return _t433
	}
	const lang = _t433.data
	...
	let _t435 = undefined
	if (eq(lang, bait__ast__Language.bait)) {
		_t435 = bait__parser__Parser_prepend_pkg(p, name)
	} else {
		_t435 = bait__ast__Language_prepend_to(lang, name)
	}
	name = _t435
	let _t436 = bait__parser__Parser_check(p, bait__token__Token.lcur)
	if (_t436.is_error) {
		return _t436
	}
_t436.data
	...
	let _t440 = bait__parser__Parser_check(p, bait__token__Token.rcur)
	if (_t440.is_error) {
		return _t440
	}
_t440.data
...
function bait__parser__Parser_global_decl(p) {
	const pos = p.pos
	let _t442 = bait__parser__Parser_check(p, bait__token__Token.key_global)
	if (_t442.is_error) {
		return _t442
	}
_t442.data
	...
	const name = bait__parser__Parser_prepend_pkg(p, _t443.data)
	let _t444 = bait__parser__Parser_check(p, bait__token__Token.decl_assign)
	if (_t444.is_error) {
		return _t444
	}
_t444.data
...
function bait__parser__Parser_interface_decl(p) {
	const pos = p.pos
	const is_pub = bait__parser__Parser_check_pub(p)
	let _t446 = bait__parser__Parser_check(p, bait__token__Token.key_interface)
	if (_t446.is_error) {
		return _t446
	}
_t446.data
	let _t447 = bait__parser__Parser_parse_lang(p)
	if (_t447.is_error) {
		return _t447
	}
	const lang = _t447.data
	let _t448 = bait__parser__Parser_check_name(p)
	if (_t448.is_error) {
		return _t448
	}
	let name = bait__ast__Language_prepend_to(lang, _t448.data)
	if (!eq(lang, bait__ast__Language.bait) && eq(p.tok, bait__token__Token.dot)) {
		...
		name = string_add(name, _t450.data)
	}
	if (bait__ast__Table_type_exists(p.table, name)) {
		Array_push(p.table.redefined_types, name)
	}
	...
	let fields = new bait_Array({ data: [], length: 0 })
	let _t452 = bait__parser__Parser_check(p, bait__token__Token.lcur)
	if (_t452.is_error) {
		return _t452
	}
_t452.data
	while (!eq(p.tok, bait__token__Token.rcur)) {
		if (eq(p.tok, bait__token__Token.key_mut)) {
			if (!eq(mut_idx, -1)) {
				let _t455 = bait__parser__Parser_error(p, from_js_string("redefinition of \"mut\" section"))
				if (_t455.is_error) {
					return _t455
				}
_t455.data
			...
			bait__parser__Parser_next(p)
			let _t456 = bait__parser__Parser_check(p, bait__token__Token.colon)
			if (_t456.is_error) {
				return _t456
			}
_t456.data
			...
			if (eq(p.tok, bait__token__Token.key_mut)) {
				if (!eq(pub_mut_idx, -1)) {
					let _t459 = bait__parser__Parser_error(p, from_js_string("redefinition of \"pub mut\" section"))
					if (_t459.is_error) {
						return _t459
					}
_t459.data
				...
				if (!eq(pub_idx, -1)) {
					let _t461 = bait__parser__Parser_error(p, from_js_string("redefinition of \"pub\" section"))
					if (_t461.is_error) {
						return _t461
					}
_t461.data
			...
			let _t462 = bait__parser__Parser_check(p, bait__token__Token.colon)
			if (_t462.is_error) {
				return _t462
			}
_t462.data
			...
			if (!eq(global_idx, -1)) {
				let _t464 = bait__parser__Parser_error(p, from_js_string("redefinition of \"global\" section"))
				if (_t464.is_error) {
					return _t464
				}
_t464.data
			...
			bait__parser__Parser_next(p)
			let _t465 = bait__parser__Parser_check(p, bait__token__Token.colon)
			if (_t465.is_error) {
				return _t465
			}
_t465.data
		...
		if (eq(p.tok, bait__token__Token.lpar)) {
			let _t468 = bait__parser__Parser_interface_method(p, fname, typ)
			if (_t468.is_error) {
				return _t468
			}
			const m = _t468.data
		...
		Array_push(fields, new bait__ast__StructField({ name: fname, typ: _t469.data, is_mut: field_is_mut, is_pub: field_is_pub, is_global: field_is_global }))
	}
	bait__parser__Parser_next(p)
...
function bait__parser__Parser_interface_method(p, name, rec_type) {
	let _t470 = bait__parser__Parser_check(p, bait__token__Token.lpar)
	if (_t470.is_error) {
		return _t470
	}
_t470.data
	let params = new bait_Array({ data: [new bait__ast__Param({ name: from_js_string("_"), typ: rec_type })], length: 1 })
	let _t471 = bait__parser__Parser_fun_params(p)
	if (_t471.is_error) {
		return _t471
	}
	Array_push_many(params, _t471.data)
	const par_line = p.pos.line
	let _t472 = bait__parser__Parser_check(p, bait__token__Token.rpar)
	if (_t472.is_error) {
		return _t472
	}
_t472.data
...
function bait__parser__Parser_return_stmt(p) {
	const pos = p.pos
	let _t475 = bait__parser__Parser_check(p, bait__token__Token.key_return)
	if (_t475.is_error) {
		return _t475
	}
_t475.data
	...
	if (!eq(p.tok, bait__token__Token.semicolon)) {
		let _t477 = bait__parser__Parser_expr(p, bait__token__Precedence.lowest)
		if (_t477.is_error) {
			return _t477
		}
		expr = _t477.data
...
function bait__parser__Parser_type_decl(p) {
	...
	const is_pub = bait__parser__Parser_check_pub(p)
	bait__parser__Parser_next(p)
	let _t478 = bait__parser__Parser_parse_lang(p)
	if (_t478.is_error) {
		return _t478
	}
	const lang = _t478.data
	let _t479 = bait__parser__Parser_check_name(p)
	if (_t479.is_error) {
		return _t479
	}
	let name = _t479.data
	...
	let _t482 = bait__parser__Parser_check(p, bait__token__Token.decl_assign)
	if (_t482.is_error) {
		return _t482
	}
_t482.data
	...
	Array_push(variants, _t483.data)
	if (is_fun_type) {
...
function bait__parser__Parser_struct_decl(p) {
	const pos = p.pos
	const is_pub = bait__parser__Parser_check_pub(p)
	let _t487 = bait__parser__Parser_check(p, bait__token__Token.key_struct)
	if (_t487.is_error) {
		return _t487
	}
_t487.data
	let _t488 = bait__parser__Parser_parse_lang(p)
	if (_t488.is_error) {
		return _t488
	}
	const lang = _t488.data
	let _t489 = bait__parser__Parser_check_name(p)
	if (_t489.is_error) {
		return _t489
	}
	let name = _t489.data
	...
	let _t491 = bait__parser__Parser_generic_type_names(p)
	if (_t491.is_error) {
		return _t491
	}
	const generic_names = _t491.data
	let mut_idx = -1
	let pub_idx = -1
	let pub_mut_idx = -1
	let global_idx = -1
...
_t492.data
	while (!eq(p.tok, bait__token__Token.rcur)) {
		if (eq(p.tok, bait__token__Token.key_mut)) {
			if (!eq(mut_idx, -1)) {
				let _t495 = bait__parser__Parser_error(p, from_js_string("redefinition of \"mut\" section"))
				if (_t495.is_error) {
					return _t495
				}
_t495.data
			...
			bait__parser__Parser_next(p)
			let _t496 = bait__parser__Parser_check(p, bait__token__Token.colon)
			if (_t496.is_error) {
				return _t496
			}
_t496.data
			...
			if (eq(p.tok, bait__token__Token.key_mut)) {
				if (!eq(pub_mut_idx, -1)) {
					let _t499 = bait__parser__Parser_error(p, from_js_string("redefinition of \"pub mut\" section"))
					if (_t499.is_error) {
						return _t499
					}
_t499.data
				...
				if (!eq(pub_idx, -1)) {
					let _t501 = bait__parser__Parser_error(p, from_js_string("redefinition of \"pub\" section"))
					if (_t501.is_error) {
						return _t501
					}
_t501.data
			...
			let _t502 = bait__parser__Parser_check(p, bait__token__Token.colon)
			if (_t502.is_error) {
				return _t502
			}
_t502.data
			...
			if (!eq(global_idx, -1)) {
				let _t504 = bait__parser__Parser_error(p, from_js_string("redefinition of \"global\" section"))
				if (_t504.is_error) {
					return _t504
				}
_t504.data
			...
			bait__parser__Parser_next(p)
			let _t505 = bait__parser__Parser_check(p, bait__token__Token.colon)
			if (_t505.is_error) {
				return _t505
			}
_t505.data
		...
		let _t506 = bait__parser__Parser_parse_attributes(p)
		if (_t506.is_error) {
			return _t506
		}
_t506.data
		let _t507 = bait__parser__Parser_struct_decl_field(p, field_is_mut, field_is_pub, field_is_global)
		if (_t507.is_error) {
			return _t507
		}
		Array_push(fields, _t507.data)
	}
	let _t508 = bait__parser__Parser_check(p, bait__token__Token.rcur)
	if (_t508.is_error) {
		return _t508
	}
_t508.data
...
function bait__parser__Parser_struct_decl_field(p, is_mut, is_pub, is_global) {
	...
	const fname = _t510.data
	let _t511 = bait__parser__Parser_parse_type(p)
	if (_t511.is_error) {
		return _t511
	}
	const ftyp = _t511.data
	let expr = new bait__ast__EmptyExpr({})
	if (eq(p.tok, bait__token__Token.decl_assign)) {
		bait__parser__Parser_next(p)
...
function bait__parser__Parser_struct_init(p) {
	const pos = p.pos
	const name = p.val
	let _t514 = bait__parser__Parser_parse_type(p)
	if (_t514.is_error) {
		return _t514
	}
	let typ = _t514.data
	let _t515 = bait__parser__Parser_concrete_types(p)
	if (_t515.is_error) {
		return _t515
	}
	const conc_types = _t515.data
	...
	let _t517 = bait__parser__Parser_check(p, bait__token__Token.lcur)
	if (_t517.is_error) {
		return _t517
	}
_t517.data
	let fields = new bait_Array({ data: [], length: 0 })
	while (!eq(p.tok, bait__token__Token.rcur)) {
		let _t518 = bait__parser__Parser_struct_init_field(p)
		if (_t518.is_error) {
			return _t518
		}
		Array_push(fields, _t518.data)
	}
	let _t519 = bait__parser__Parser_check(p, bait__token__Token.rcur)
	if (_t519.is_error) {
		return _t519
	}
_t519.data
...
function bait__parser__Parser_struct_init_field(p) {
	...
	const name = _t520.data
	let _t521 = bait__parser__Parser_check(p, bait__token__Token.assign)
	if (_t521.is_error) {
		return _t521
	}
_t521.data
...
function bait__parser__Parser_parse_type(p) {
	...
	if (eq(p.tok, bait__token__Token.lsqr)) {
		bait__parser__Parser_next(p)
		let _t527 = bait__parser__Parser_check(p, bait__token__Token.rsqr)
		if (_t527.is_error) {
			return _t527
		}
_t527.data
		...
		const elem_type = _t528.data
		return new Result({ data: bait__ast__Table_find_or_register_array(p.table, elem_type) })
	}
	if (eq(p.tok, bait__token__Token.key_fun)) {
		let _t530 = bait__parser__Parser_parse_fun_type(p)
		if (_t530.is_error) {
			return _t530
		}
		return new Result({ data: _t530.data })
	}
	if (string_eq(p.val, from_js_string("map"))) {
		bait__parser__Parser_next(p)
		let _t532 = bait__parser__Parser_parse_map_type(p)
		if (_t532.is_error) {
			return _t532
		}
	...
	let _t534 = bait__parser__Parser_parse_lang(p)
	if (_t534.is_error) {
		return _t534
	}
	const lang = _t534.data
	let _t535 = bait__parser__Parser_parse_name_type(p, lang)
	if (_t535.is_error) {
		return _t535
	}
	let typ = _t535.data
...
function bait__parser__Parser_parse_name_type(p, lang) {
	...
	let name = bait__ast__Language_prepend_to(lang, _t538.data)
	if (eq(p.tok, bait__token__Token.dot)) {
		if (eq(lang, bait__ast__Language.js)) {
			bait__parser__Parser_next(p)
			let _t541 = bait__parser__Parser_check_name(p)
			if (_t541.is_error) {
				return _t541
			}
			name = string_add(name, string_add(from_js_string("."), _t541.data))
		} else {
			...
			name = string_add(string_add(pkg, from_js_string(".")), _t542.data)
		}
...
_t547.data
	...
	const par_line = p.pos.line
	let _t548 = bait__parser__Parser_check(p, bait__token__Token.rpar)
	if (_t548.is_error) {
		return _t548
	}
_t548.data
...
function bait__parser__Parser_parse_map_type(p) {
	...
	let _t552 = bait__parser__Parser_check(p, bait__token__Token.lsqr)
	if (_t552.is_error) {
		return _t552
	}
_t552.data
	...
	const key_type = _t553.data
	if (!eq(key_type, bait__ast__STRING_TYPE)) {
		let _t555 = bait__parser__Parser_error(p, from_js_string("map key type must be string"))
		if (_t555.is_error) {
			return _t555
		}
_t555.data
	}
	let _t556 = bait__parser__Parser_check(p, bait__token__Token.rsqr)
	if (_t556.is_error) {
		return _t556
	}
_t556.data
...
function bait__parser__Parser_generic_type_names(p) {
	...
	while (true) {
		let _t559 = bait__parser__Parser_check_name(p)
		if (_t559.is_error) {
			return _t559
		}
		const name = _t559.data
		if (i32(name.length > 1)) {
			let _t561 = bait__parser__Parser_error(p, from_js_string("generic types names have to be exactly one character"))
			if (_t561.is_error) {
				return _t561
			}
_t561.data
		...
		if (!u8_is_upper(string_get(name, 0))) {
			let _t563 = bait__parser__Parser_error(p, from_js_string("generic type names have to be capital letters"))
			if (_t563.is_error) {
				return _t563
			}
_t563.data
		...
		let _t566 = bait__parser__Parser_check(p, bait__token__Token.comma)
		if (_t566.is_error) {
			return _t566
		}
_t566.data
...
function bait__parser__Parser_concrete_types(p) {
	...
	while (!eq(p.tok, bait__token__Token.rsqr)) {
		...
		Array_push(types, _t568.data)
		if (!eq(p.tok, bait__token__Token.rsqr)) {
			let _t570 = bait__parser__Parser_check(p, bait__token__Token.comma)
			if (_t570.is_error) {
				return _t570
			}
_t570.data
...
function bait__checker__Checker_decl_assign(c, node) {
	...
	if (bait__ast__Scope_is_known(c.scope, left.name)) {
		...
		if (eq(obj.kind, bait__ast__ObjectKind.package_)) {
			_t576 = from_js_string(`cannot shadow import "${left.name.str}"`)
		} else {
			_t576 = from_js_string(`redefinition of ${left.name.str}`)
		}
		const msg = _t576
...
function bait__checker__Checker_check_fun_attrs(c, node) {
	for (let _t590 = 0; _t590 < node.attrs.length; _t590++) {
		const attr = Array_get(node.attrs, _t590)
...
function bait__checker__Checker_check_fun_attrs_on_call(c, call, def) {
	...
	for (let _t599 = 0; _t599 < def.attrs.length; _t599++) {
		const attr = Array_get(def.attrs, _t599)
...
function bait__checker__Checker_check_struct_field_attrs(c, node) {
	for (let _t605 = 0; _t605 < node.fields.length; _t605++) {
		const field = Array_get(node.fields, _t605)
		for (let _t606 = 0; _t606 < field.attrs.length; _t606++) {
			const attr = Array_get(field.attrs, _t606)
...
function bait__checker__Checker_check_files(c, files) {
	for (let _t609 = 0; _t609 < files.length; _t609++) {
		const f = Array_get(files, _t609)
	...
	while (c.need_generic_resolve && i32(resolve_runs < 100)) {
		...
		for (let _t610 = 0; _t610 < files.length; _t610++) {
			const f = Array_get(files, _t610)
...
function bait__checker__Checker_expr(c, expr) {
	...
	if (expr instanceof bait__ast__AnonFun) {
		...
		_t620 = panic(from_js_string(`unexpected EmptyExpr at ${bait__token__Pos_str(expr.pos).str}`))
	}
	const t = _t620
...
function bait__checker__Checker_string_inter_literal(c, node) {
	for (let _t664 = 0; _t664 < node.exprs.length; _t664++) {
		const e = Array_get(node.exprs, _t664)
		const typ = bait__checker__Checker_expr(c, e)
...
function bait__checker__Checker_fun_decl(c, node) {
	...
	if (node.is_method) {
		...
		let param_types = new bait_Array({ data: [], length: 0 })
		for (let _t671 = 0; _t671 < node.params.length; _t671++) {
			const param = Array_get(node.params, _t671)
			Array_push(param_types, param.typ)
		}
...
function bait__checker__Checker_fun_params(c, params) {
	for (let _t675 = 0; _t675 < params.length; _t675++) {
		...
		if (bait__ast__Scope_is_known(c.scope, p.name)) {
			...
			let _t677 = undefined
			if (eq(obj.kind, bait__ast__ObjectKind.package_)) {
				_t677 = from_js_string(`cannot shadow import "${p.name.str}"`)
			} else {
				_t677 = from_js_string(`redefinition of ${p.name.str}`)
			}
			const msg = _t677
...
function bait__checker__Checker_check_main_fun(c, stmts) {
	for (let _t680 = 0; _t680 < stmts.length; _t680++) {
		const stmt = Array_get(stmts, _t680)
...
function bait__checker__Checker_resolve_generics_funs(c) {
	for (let _t730 = 0; _t730 < c.file.generic_funs.length; _t730++) {
		const fn = Array_get(c.file.generic_funs, _t730)
		const gtypes = map_get_set(c.table.generic_fun_types, bait__ast__FunDecl_key(fn), new bait_Array({ data: [], length: 0 }))
		for (let _t731 = 0; _t731 < gtypes.length; _t731++) {
			const concrete = Array_get(gtypes, _t731)
...
function bait__checker__Checker_check_exhaustive_match(c, info, branch_exprs, node) {
	...
	if (info instanceof bait__ast__EnumInfo) {
		for (let _t751 = 0; _t751 < info.vals.length; _t751++) {
			const val = Array_get(info.vals, _t751)
...
function bait__checker__stmt_is_redef(stmt, name, kind) {
	...
	if (eq(kind, bait__checker__RedefKind.fun_)) {
		_t770 = stmt instanceof bait__ast__FunDecl && string_eq(stmt.name, name)
	} else if (eq(kind, bait__checker__RedefKind.type_)) {
		_t770 = (stmt instanceof bait__ast__EnumDecl || stmt instanceof bait__ast__InterfaceDecl || stmt instanceof bait__ast__StructDecl || stmt instanceof bait__ast__TypeDecl) && string_eq(stmt.name, name)
	}
	return _t770
}
...
function bait__checker__Checker_check_redefinitions(c, redefs_list, kind, files) {
	...
	for (let _t772 = 0; _t772 < redefs_list.length; _t772++) {
		const name = Array_get(redefs_list, _t772)
		if (!Array_contains_string(unique, name)) {
			Array_push(unique, name)
		}
	}
	for (let _t774 = 0; _t774 < unique.length; _t774++) {
		const name = Array_get(unique, _t774)
		let redefinitions = new bait_Array({ data: [], length: 0 })
		for (let _t775 = 0; _t775 < files.length; _t775++) {
			const file = Array_get(files, _t775)
			for (let _t776 = 0; _t776 < file.stmts.length; _t776++) {
				const stmt = Array_get(file.stmts, _t776)
		...
		for (let _t778 = 0; _t778 < redefinitions.length; _t778++) {
			const redef = Array_get(redefinitions, _t778)
...
function bait__checker__Checker_struct_init(c, node) {
	...
	node.name = sym.name
	for (let _t819 = 0; _t819 < node.fields.length; _t819++) {
		const field = Array_get(node.fields, _t819)
...
function bait__checker__Checker_check_init_field_values(c, init, info) {
	for (let _t823 = 0; _t823 < info.fields.length; _t823++) {
		const def_field = Array_get(info.fields, _t823)
		const def_sym = bait__ast__Table_get_sym(c.table, def_field.typ)
		let is_required = eq(def_sym.kind, bait__ast__TypeKind.sum_type)
		for (let _t824 = 0; _t824 < def_field.attrs.length; _t824++) {
			const attr = Array_get(def_field.attrs, _t824)
		...
		for (let _t828 = 0; _t828 < init.fields.length; _t828++) {
			const inited = Array_get(init.fields, _t828)
...
function bait__checker__Checker_does_type_exist(c, sym, pos) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.sum_type)) {
		const info = sym.info
		for (let _t854 = 0; _t854 < info.variants.length; _t854++) {
			const variant = Array_get(info.variants, _t854)
	...
	if (eq(sym.kind, bait__ast__TypeKind.fun_)) {
		const info = sym.info
		for (let _t857 = 0; _t857 < info.param_types.length; _t857++) {
			const param_type = Array_get(info.param_types, _t857)
...
function bait__util__escape_linebreak(s) {
	let _t863 = undefined
	if (string_eq(os__platform(), from_js_string("win32"))) {
		_t863 = string_replace(s, from_js_string("\r\n"), from_js_string("\\r\\n"))
	} else {
		_t863 = string_replace(s, from_js_string("\n"), from_js_string("\\n"))
	}
	return _t863
...
const bait__util__FULL_VERSION = from_js_string(`${bait__util__VERSION.str} ${from_js_string("8079274").str}`)
...
function bait__gen__js__Gen_get_comptime_val(g, name, pos) {
	...
	if (string_eq(name, from_js_string("PKG"))) {
		...
		_t864 = string_replace(os__abs_path(g.path), from_js_string("\\"), from_js_string("\\\\"))
	} else if (string_eq(name, from_js_string("FILE"))) {
		_t864 = string_replace(g.path, from_js_string("\\"), from_js_string("\\\\"))
	} else if (string_eq(name, from_js_string("LINE"))) {
		_t864 = i32_str(pos.line)
		...
		_t864 = from_js_string(`${file.str}:${line.str}`)
	} else if (string_eq(name, from_js_string("FUN"))) {
		_t864 = g.cur_fun.name
	} else if (string_eq(name, from_js_string("BAITEXE"))) {
		_t864 = bait__gen__js__Gen_comptime_baitexe(g)
	} else if (string_eq(name, from_js_string("BAITDIR"))) {
		_t864 = bait__gen__js__Gen_comptime_baitdir(g)
	} else if (string_eq(name, from_js_string("BAITHASH"))) {
		_t864 = bait__gen__js__Gen_comptime_baithash(g)
	} else {
		_t864 = from_js_string("")
	}
	return _t864
}
...
function bait__gen__js__Gen_fun_decl(g, node) {
	...
	if (i32(node.generic_names.length > 0) && eq(g.cur_concrete_types.length, 0)) {
		...
		for (let _t899 = 0; _t899 < gtypes.length; _t899++) {
			const conc_types = Array_get(gtypes, _t899)
...
function bait__gen__js__gen(files, table, pref) {
	...
	for (let _t928 = 0; _t928 < files.length; _t928++) {
		const file = Array_get(files, _t928)
		g.path = file.path
		g.pkg = file.pkg_name
	...
	for (let _t931 = 0; _t931 < g.table.needed_str_funs.length; _t931++) {
		const typ = Array_get(g.table.needed_str_funs, _t931)
...
function bait__gen__js__Gen_cut_before_any(g, chars) {
	for (let i = i32(g.out.length - 1); i32(i >= 0); i = i32(i - 1)) {
		for (let _t938 = 0; _t938 < chars.length; _t938++) {
			const c = string_get(chars, _t938)
...
function bait__gen__js__Gen_gen_test_main(g) {
	...
	const _t941 = map_keys(g.table.fun_decls)
	for (let _t942 = 0; _t942 < _t941.length; _t942++) {
		const key = Array_get(_t941, _t942)
...
function bait__gen__js__Gen_get_concrete_name(g, name, concrete_types) {
	let full_name = name
	for (let _t948 = 0; _t948 < concrete_types.length; _t948++) {
		const t = Array_get(concrete_types, _t948)
...
function bait__gen__js__Gen_stmts(g, stmts) {
	bait__gen__js__Gen_save_stmt_offset(g)
	g.indent = i32(g.indent + 1)
	for (let _t951 = 0; _t951 < stmts.length; _t951++) {
		const stmt = Array_get(stmts, _t951)
...
function bait__gen__js__Gen_enum_decl(g, node) {
	...
	g.indent = i32(g.indent + 1)
	for (let _t965 = 0; _t965 < node.fields.length; _t965++) {
		const field = Array_get(node.fields, _t965)
...
let _t974 = undefined
if (string_eq(os__platform(), from_js_string("win32"))) {
	_t974 = from_js_string("\\r\\n")
} else {
	_t974 = from_js_string("\\n")
}
const bait__gen__js__LB = _t974
...
function bait__gen__js__Gen_generate_str_fun(g, typ) {
	...
	let s = "${sym.name.str}{"\n`))
		...
		for (let _t980 = 0; _t980 < info.fields.length; _t980++) {
			const field = Array_get(info.fields, _t980)
			if (eq(typ, field.typ)) {
	...
	if (eq(sym.kind, bait__ast__TypeKind.sum_type)) {
		const info = sym.info
		g.fun_decls_out = string_add(g.fun_decls_out, from_js_string(`function ${name.str}(it, indent) {\n`))
		for (let _t985 = 0; _t985 < info.variants.length; _t985++) {
			const bait_var = Array_get(info.variants, _t985)
...
`))
	...
	switch(it) {\n`))
		const info = sym.info
		for (let _t987 = 0; _t987 < info.vals.length; _t987++) {
			const val = Array_get(info.vals, _t987)
...
function bait__gen__c__gen(files, table, pref) {
	...
	for (let _t1000 = 0; _t1000 < files.length; _t1000++) {
		const file = Array_get(files, _t1000)
		g.path = file.path
		g.pkg = file.pkg_name
...
function bait__gen__c__Gen_gen_test_main(g) {
	...
	const _t1007 = map_keys(g.table.fun_decls)
	for (let _t1008 = 0; _t1008 < _t1007.length; _t1008++) {
		const key = Array_get(_t1007, _t1008)
		const func = map_get(g.table.fun_decls, key)
		if (func.is_test) {
			nr_test_funs = i32(nr_test_funs + 1)
...
function bait__gen__c__Gen_cut_before_any(g, chars) {
	for (let i = i32(g.out.length - 1); i32(i >= 0); i = i32(i - 1)) {
		for (let _t1015 = 0; _t1015 < chars.length; _t1015++) {
			const c = string_get(chars, _t1015)
			if (eq(string_get(g.out, i), c)) {
				let cut = string_substr(g.out, i32(i + 1), g.out.length)
				g.out = string_substr(g.out, 0, i)
				return cut
...
function bait__gen__c__Gen_get_comptime_val(g, name, pos) {
	let _t1018 = undefined
	if (string_eq(name, from_js_string("PKG"))) {
		_t1018 = g.pkg
	} else if (string_eq(name, from_js_string("ABS_FILE"))) {
		_t1018 = string_replace(os__abs_path(g.path), from_js_string("\\"), from_js_string("\\\\"))
	} else if (string_eq(name, from_js_string("FILE"))) {
		_t1018 = string_replace(g.path, from_js_string("\\"), from_js_string("\\\\"))
	} else if (string_eq(name, from_js_string("LINE"))) {
		_t1018 = i32_str(pos.line)
	} else if (string_eq(name, from_js_string("FILE_LINE"))) {
		...
		_t1018 = from_js_string(`${file.str}:${line.str}`)
	} else if (string_eq(name, from_js_string("FUN"))) {
		_t1018 = g.cur_fun.name
	} else if (string_eq(name, from_js_string("BAITEXE"))) {
		_t1018 = bait__gen__c__Gen_comptime_baitexe(g)
	} else if (string_eq(name, from_js_string("BAITDIR"))) {
		_t1018 = bait__gen__c__Gen_comptime_baitdir(g)
	} else if (string_eq(name, from_js_string("BAITHASH"))) {
		_t1018 = bait__gen__c__Gen_comptime_baithash(g)
	} else {
		_t1018 = from_js_string("")
	}
	return _t1018
}
...
function bait__gen__c__Gen_gen_equality_funs(g) {
	for (let _t1022 = 0; _t1022 < g.table.needed_equality_funs.length; _t1022++) {
		const typ = Array_get(g.table.needed_equality_funs, _t1022)
...
function bait__gen__c__Gen_array_init(g, node) {
	...
	g.indent = i32(g.indent + 1)
	for (let _t1031 = 0; _t1031 < node.exprs.length; _t1031++) {
		const expr = Array_get(node.exprs, _t1031)
		bait__gen__c__Gen_expr(g, expr)
		bait__gen__c__Gen_write(g, from_js_string(", "))
	}
	g.indent = i32(g.indent - 1)
...
function bait__gen__c__Gen_typ(g, typ) {
	...
	let _t1094 = undefined
	if (eq(sym.kind, bait__ast__TypeKind.enum_)) {
		_t1094 = from_js_string("enum ")
	} else {
		_t1094 = from_js_string("")
	}
	const prefix = _t1094
...
function bait__gen__c__Gen_write_types(g) {
	for (let _t1095 = 0; _t1095 < g.table.type_symbols.length; _t1095++) {
		const sym = Array_get(g.table.type_symbols, _t1095)
		...
		if (sym.info instanceof bait__ast__StructInfo) {
			...
			for (let _t1098 = 0; _t1098 < info.fields.length; _t1098++) {
				const field = Array_get(info.fields, _t1098)
			...
			for (let _t1099 = 0; _t1099 < info.vals.length; _t1099++) {
				const val = Array_get(info.vals, _t1099)
...
function bait__builder__Builder_bait_files_in_dir(b, dir) {
	...
	for (let _t1102 = 0; _t1102 < all_files.length; _t1102++) {
		const f = Array_get(all_files, _t1102)
...
function bait__builder__compile(prefs) {
	...
	for (let _t1108 = 0; _t1108 < paths.length; _t1108++) {
		const p = Array_get(paths, _t1108)
	...
	for (let i = 0; i32(i < ast_files.length); i = i32(i + 1)) {
		...
		for (let _t1109 = 0; _t1109 < file.imports.length; _t1109++) {
			const imp = Array_get(file.imports, _t1109)
			...
			for (let _t1114 = 0; _t1114 < imp_paths.length; _t1114++) {
				const p = Array_get(imp_paths, _t1114)
	...
	for (let _t1118 = 0; _t1118 < ast_files.length; _t1118++) {
		const f = Array_get(ast_files, _t1118)
		for (let _t1119 = 0; _t1119 < f.imports.length; _t1119++) {
			const imp = Array_get(f.imports, _t1119)
			if (!eq(imp.lang, bait__ast__Language.bait)) {
				continue
			}
	...
	for (let _t1121 = 0; _t1121 < pkg_order.length; _t1121++) {
		const pkg = Array_get(pkg_order, _t1121)
...
function bait__builder__Builder_print_errors_and_warnings(b, parser_errs) {
	...
	for (let _t1136 = 0; _t1136 < b.parsed_files.length; _t1136++) {
		...
		for (let _t1140 = 0; _t1140 < f.errors.length; _t1140++) {
			const err = Array_get(f.errors, _t1140)
	...
	for (let _t1142 = 0; _t1142 < b.checker.errors.length; _t1142++) {
		const err = Array_get(b.checker.errors, _t1142)
		bait__errors__Message_print(err)
	}
...
function bait__builder__run_tests(prefs) {
	...
	for (let _t1152 = 0; _t1152 < prefs.args.length; _t1152++) {
		const a = Array_get(prefs.args, _t1152)

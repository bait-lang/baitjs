...
_t432.data
	if (!eq(lang, bait__ast__Language.bait)) {
		...
		bait__ast__Scope_register(p.table.ffi_scope, bait__ast__Language_prepend_to(lang, name), new bait__ast__ScopeObject({ typ: typ, kind: bait__ast__ObjectKind.constant, is_pub: true }))
	...
	bait__ast__Scope_register(Map_get_set(p.table.scopes, p.pkg_name, null), name, new bait__ast__ScopeObject({ typ: typ, kind: bait__ast__ObjectKind.constant, is_pub: is_pub, pkg: p.pkg_name, expr: expr }))
...
const bait__util__FULL_VERSION = from_js_string(`${bait__util__VERSION.str} ${from_js_string("1d9ff55").str}`)
